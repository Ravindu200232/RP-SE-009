export interface ImplementationPlanDiagnostics {
  valid: boolean;
  issues: string[];
  missingHeadings: string[];
  routeHeaderOk: boolean;
  pageRoutes: string[];
  apiRowsInPageTable: string[];
  apiRoutes: string[];
  pageBlueprintRoutes: string[];
  missingPageBlueprintRoutes: string[];
  expectedPageRoutes: string[];
  missingExpectedPageRoutes: string[];
  expectedModels: string[];
  documentedModels: string[];
  missingModels: string[];
  missingPageBlueprintLabels: string[];
...
function toPascalName(raw: string): string {
  const cleaned = raw
    .replace(/[`*_()[\]{}]/g, ' ')
...
function addModelCandidate(out: string[], raw: string) {
  const candidate = toPascalName(raw);
  if (looksLikeModelName(candidate)) out.push(candidate);
}

export function extractExpectedModels(srs: string): string[] {
  ...
  for (const originalLine of lines) {
    ...
    if (!line) {
      if (inSchemaSection) schemaBudget -= 1;
      if (schemaBudget <= 0) inSchemaSection = false;
      continue;
    }

    const sectionStart = line.match(
      /^(?:#+\s*)?(?:database\s+collections?|database\s+schema|data\s+models?|models?|entities|collections?)\s*:?\s*(.*)$/i,
    );
    if (sectionStart) {
    ...
    if (
      inSchemaSection &&
      /^(?:expected\s+apis?|api\s+routes?|pages?|public\s+pages?|dashboard\s+pages?|build\s+quality|quality|roles?|auth|goal|tech\s+stack)\s*:/i.test(
        line,
      )
    ) {
...
function compactPagePhrase(raw: string): string {
  return raw
    .replace(/\([^)]*\)/g, ' ')
...
function splitPageItems(raw: string): string[] {
...
function routeForPagePhrase(phrase: string, scope: 'public' | 'dashboard' | 'auth' | 'generic'): string {
  const lower = phrase.toLowerCase();
  if (/\blogin\b|sign in/.test(lower)) return '/login';
  if (/\bregister\b|sign up/.test(lower)) return '/register';
  if (/\bhome\b|landing/.test(lower)) return '/';
  if (/product\s+details?|item\s+details?/.test(lower)) return '/products/[slug]';
  if (/product\s+listing|products?\s+list|catalog/.test(lower)) return '/products';
  if (/\bcart\b/.test(lower)) return '/cart';
  if (/\bcheckout\b/.test(lower)) return '/checkout';
  if (/order\s+success|success/.test(lower)) return '/order-success';
  if (/\babout\b/.test(lower)) return '/about';
  if (/\bcontact\b/.test(lower)) return '/contact';

  const isDashboard = scope === 'dashboard' || /dashboard|admin|manager|staff|reports?/i.test(phrase);
  if (isDashboard && /overview|dashboard/.test(lower)) return '/dashboard';
  if (isDashboard && /\bpos\b|billing|cashier/.test(lower)) return '/dashboard/pos';
  if (isDashboard && /stock\s+adjust/.test(lower)) return '/dashboard/stock-adjustments';
  if (isDashboard && /\bgrn\b|receiving|goods\s+received/.test(lower)) return '/dashboard/grn';
  if (isDashboard && /online\s+orders?|orders?/.test(lower)) return '/dashboard/orders';
  if (isDashboard && /invoice|sales?\s+history|sales?/.test(lower)) return '/dashboard/sales';
  if (isDashboard && /reports?/.test(lower)) return '/dashboard/reports';
  if (isDashboard && /settings?/.test(lower)) return '/dashboard/settings';
  if (isDashboard && /staff|users?|roles?/.test(lower)) return '/dashboard/users';
...
function pageFromPhrase(
  phrase: string,
  scope: 'public' | 'dashboard' | 'auth' | 'generic',
): ExpectedPage {
...
export function extractExpectedPages(srs: string): ExpectedPage[] {
  const out: ExpectedPage[] = [];
  for (const line of (srs || '').replace(/\r\n/g, '\n').split('\n')) {
    ...
    for (const item of splitPageItems(match[2])) out.push(pageFromPhrase(item, scope));
  }

  const lower = srs.toLowerCase();
  if (/\blogin page\b|\blogin\b/.test(lower)) out.push(pageFromPhrase('login', 'auth'));
...
function routeRowsFromPlan(plan: string): Array<{ route: string; page: string; purpose: string }> {
  const rows: Array<{ route: string; page: string; purpose: string }> = [];
  for (const match of plan.matchAll(/^\|\s*`?(\/[^|` ]*)`?\s*\|\s*([^|]+)\|(?:\s*([^|]+)\|){0,3}\s*([^|]*)\|?$/gm)) {
    const route = match[1].trim();
    if (route === '/---' || route.toLowerCase() === '/route') continue;
    rows.push({
      route,
...
function pageBlueprintRoutesFromPlan(plan: string): string[] {
  return unique(
    [...plan.matchAll(/^###\s+`?(\/[^`\s]*)`?\s+-\s+([^\n]+)$/gm)]
      .map((match) => match[1].trim())
      .filter((route) => !route.startsWith('/api')),
  );
}

function documentedModelsFromPlan(plan: string): string[] {
  return unique(
    [...plan.matchAll(/^###\s+([A-Z][A-Za-z0-9]+)\s*\((?:lib\/models\/|models\/)[^)]+\.ts\)/gm)].map(
      (match) => match[1],
    ),
  );
}

function apiRoutesFromPlan(plan: string): string[] {
  return unique(
    [...plan.matchAll(/\b(GET|POST|PUT|PATCH|DELETE)\s+(\/api\/[A-Za-z0-9_\-/:[\]\{\}]+)/g)].map(
      (match) => `${match[1]} ${match[2]}`,
    ),
  );
}

function expectedApiMinimum(srs: string, hasBackend: boolean, expectedModels: string[]): number {
  if (!hasBackend) return 0;
  const lower = srs.toLowerCase();
  if (!/(api|crud|database|mongodb|mongoose|auth|login|register|backend)/.test(lower)) return 0;
  if (expectedModels.length >= 5) return 10;
  if (expectedModels.length >= 2) return Math.min(6, expectedModels.length * 2);
  return 2;
}

export function analyzeImplementationPlan(
  plan: string,
  srs: string,
  hasBackend: boolean,
): ImplementationPlanDiagnostics {
  ...
  const missingExpectedPageRoutes = expectedPageRoutes.filter(
    (route) => !pageRoutes.includes(route),
  );
  const pageBlueprintRoutes = pageBlueprintRoutesFromPlan(plan);
  const missingPageBlueprintRoutes = pageRoutes.filter(
    (route) => !pageBlueprintRoutes.includes(route),
  );
  ...
  const issues: string[] = [];
  if (missingHeadings.length) issues.push(`Missing headings: ${missingHeadings.join(', ')}`);
  if (!routeHeaderOk) issues.push('Page/Module List table header is not exact.');
  ...
  if (codeFenceCount) issues.push('Markdown code fences are present.');
  if (hasTypeScriptInterfaces) issues.push('TypeScript interfaces/code snippets are present.');
...
export function formatPlanDiagnosticsForPrompt(diagnostics: ImplementationPlanDiagnostics): string {
...
function sectionInsertBefore(plan: string, heading: string, addition: string): string {
  const index = plan.indexOf(heading);
  if (index < 0) return `${plan.trim()}\n\n${addition.trim()}\n`;
  return `${plan.slice(0, index).trimEnd()}\n\n${addition.trim()}\n\n${plan.slice(index).trimStart()}`;
}

function sourceLineForModel(srs: string, model: string): string {
  const pattern = new RegExp(`^\\s*(?:[-*]\\s*)?${escapeRegExp(model)}\\s*[:(-]\\s*(.*)$`, 'im');
  const match = srs.match(pattern);
  return match?.[1]?.trim() || '';
}

function fieldsForModel(srs: string, model: string): string[] {
  ...
  const fields = [...source.matchAll(/\b[A-Za-z_][A-Za-z0-9_]*\b/g)]
    ...
    .filter((word) => {
      ...
      if (FIELD_STOP_WORDS.has(lower)) return false;
      if (lower === model.toLowerCase()) return false;
...
function typeForField(field: string): string {
  const lower = field.toLowerCase();
  if (lower.endsWith('id') || lower === 'id') return 'ObjectId';
  if (/(price|amount|total|quantity|points|tax|discount|level|subtotal|grandtotal)/.test(lower)) {
    return 'Number';
  }
  if (/(images|items|roles|permissions)/.test(lower)) return 'Array';
  if (/(createdat|updatedat|receivedat|date|expiresat)/.test(lower)) return 'Date';
  if (/(active|enabled|verified)/.test(lower)) return 'Boolean';
  return 'String';
}

function modelBlock(model: string, srs: string): string {
...
function kebabModelRoute(model: string): string {
...
function apiLinesForModels(models: string[], existingApis: string[]): string[] {
...
function pageRow(page: ExpectedPage): string {
  return `| ${page.route} | ${page.title} | ${page.type} | ${page.access} | ${page.purpose} |`;
}

function insertPageRows(plan: string, pages: ExpectedPage[]): string {
  if (!pages.length) return plan;
  ...
  let headerIndex = lines.findIndex((line) =>
    /\|\s*Route\s*\|\s*Page\s*\|\s*Type\s*\|\s*Access\s*\|\s*Purpose\s*\|/.test(line),
  );
  ...
  if (headerIndex < 0) {
    ...
    if (lines[insertAt]?.trim().startsWith('|')) insertAt += 1;
...
function pageBlock(route: string, title: string, purpose: string): string {
...
export function normalizeImplementationPlanCoverage(
  plan: string,
  srs: string,
  hasBackend: boolean,
): string {
  ...
  if (needsApis || diagnostics.apiRowsInPageTable.length) {
    ...
    if (addition) out = sectionInsertBefore(out, '## UI and Component Strategy', addition);
  }
