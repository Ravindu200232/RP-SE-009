...
`domains.py` knows what a hotel or a dealership is made of. This module knows
what a POS or a landing page is *shaped* like: which archetype generates it,
whether it has a sidebar, what its default pages are, and what palette it wears.

The two are combined by `build_pack()`, which is what seeds the interview's
suggested answers and, later, the SRS skeleton.

Archetypes:
  fullstack-crud       admin shell + one CRUD folder per entity
  public-content       a published site with an author admin behind it
  landing-single-page  one route, N marketing sections
  single-tool          one focused tool page, optional storage
"""
...
APP_TYPES: dict[str, dict[str, Any]] = {
    "pos": {
        ...
        "keywords": ["pos", "point of sale", "cashier", "billing", "invoice", "grn",
                     ...
                     # Not "ring up": in half the country that means telephone,
                     # and "people ring up and we write the order down" is the
                     # dashboard case exactly.
...
def resolve_archetype(text: str, fallback: str = "fullstack-crud") -> str:
    ...
    "Other" must never produce an architecture nothing downstream understands,
    so it resolves here rather than being carried as an unknown.
    """
...
def guess_app_type(text: str) -> tuple[str, float]:
    ...
    Same scoring shape as domains.classify_domain: multi-word phrases count
    double because they are far stronger evidence than a single token.

    Two things here are load-bearing, and both were wrong.

    Nothing matching returns "other", not `DEFAULT_APP_TYPE`. The old pair —
    ("saas", 0.0) — names a category while saying it has no evidence for it,
    which is an invitation to read the first half and drop the second, and that
    is exactly what `record` did. "other" is the honest answer and it is
    also the safe one: `landing`, `portfolio` and `utility` carry
    `auth_policy: "disabled"`, so a no-evidence guess must never be able to
    land on one and silently remove the login.

    Confidence now counts how much evidence there is, not only which category
    holds the largest share of it. One token used to be enough for 0.97 as long
    as no other category scored — so "a barber shop" came back ecommerce at
    0.97 on the word "shop", and a photo studio with two assistants came back
    landing at 0.97 on "studio", which switches authentication off. A lone
    single-word hit is now 0.58, below GUESS_FLOOR; a phrase, or two words
    agreeing, clears it.
    """
    low = (text or "").lower()
...
def build_pack(app_type_key: str, idea: str = "") -> dict[str, Any]:
    ...
    The result is the "pack" every interview question is conditioned on: it
    supplies the suggested roles, entities and features so the model is choosing
    from real options rather than inventing from nothing.
    """
...
def _singular_pascal(table_name: str) -> str:
    """`sales_orders` -> `SalesOrder`. Naive, but these are only suggestions the
    user confirms in the interview."""
