...
def refresh_handoff(handoff: dict, plan: dict, srs_document: dict) -> dict:
    ...
    Approved SRS projects outlive the code that first generated them.  Reading
    `/builder-handoff` therefore upgrades the whole object, not only its prose.
    Unknown top-level keys from the stored object are preserved.
    """
    plan = dict(plan or {})
    try:
