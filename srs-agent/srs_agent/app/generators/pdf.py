...
from .standards import (
    SRS_STANDARD, SRS_STANDARD_TITLE, UML_STANDARD, BPMN_STANDARD,
    DIAGRAM_GUIDANCE, DIAGRAM_NOTATION,
)
...
def build_srs_pdf(srs: dict, out_path: str | Path, *, status: str = "Draft",
    ...
    for idx, d in enumerate(diagrams):
        ...
        guide = DIAGRAM_GUIDANCE.get(d.get("kind"), {})
        question = d.get("question") or guide.get("question")
        definition = d.get("definition") or guide.get("definition")
        drawing_rules = d.get("drawing_rules") or guide.get("drawing_rules") or []
        if question:
            story.append(_P(f"<b>{question}</b>", st, "body"))
        if definition:
            story.append(_P(str(definition), st, "small"))
        if drawing_rules:
            story.append(_P("<b>How this diagram starts</b>", st, "small"))
            for item in drawing_rules:
        ...
        notation = d.get("notation") or DIAGRAM_NOTATION.get(d.get("kind"), [])
        # Keep the notation legend on the same page.
        reserve = 8.4 * cm + (len(notation) + len(drawing_rules)) * 0.43 * cm
