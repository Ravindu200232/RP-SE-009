...
def _first_lifecycle(doc: dict) -> dict:
    ...
    Multiple entities often have status enums.  Prefer the one for which an FR
    or workflow actually states a `from X to Y` transition; otherwise return the
    first lifecycle candidate without inventing any transition semantics.
    """
    candidates = []
...
def bpmn_diagram(doc: dict) -> str:
    """Mermaid preview of BPMN semantics; native SVG is the canonical BPMN rendering."""
    ...
    roles = [str(r.get("role_name") or r.get("role_key")) for r in (doc.get("roles") or []) if (r.get("role_name") or r.get("role_key"))][:3] or ["Participant"]
