...
def _access_section(doc: dict, plan: dict) -> list[str]:
    ...
    The handoff used to say something about roles only when there were NONE
    ("no accounts, every page is public"). An app WITH roles got pages, data
    and a flat requirement list in which the access rules were scattered as
    ordinary sentences — "Managers can see the money" sitting between "Users
    can edit a booking" and "A Checkout page". Nothing said which routes were
    protected, nothing said what a signed-out visitor gets, and nothing said
    the check has to run on the server. So it was built the way that reads:
    every page reachable, the role used to hide a nav link.

    Read from the SRS rather than the plan, because `role_access_matrix` and
    `protected_pages` are where the interview's answers actually land.
    """
    doc = doc or {}
    lines: list[str] = []
...
def _routes_for(step: str, pages: list[dict], who: str = "",
    ...
    """The routes of the pages a workflow step is talking about, in the
    order the step mentions them.

    Full page names first — "Order History" in "views the order status in
    Order History" — and in the order they appear, because one step often
    walks two pages ("browses the catalogue and adds a book to the cart" is
    Catalogue then Cart). Where no full name appears, the pages whose own
    words the step uses: a page counts if the step uses ALL its words, or its
    head noun when no other page shares that noun ("cart" is only Shopping
    Cart; "book" is three pages, so "book" alone names none of them). Ties
    go to a page the journey's role can see — the owner's "manage customer
    orders" is Admin Orders, not the customer's Order History. Measured on
    the book shop before this: the buying journey came out as "/checkout →
    /order-history" because its first two steps named no page in full.
    """
    text = (step or "").lower()
...
def _flow_section(plan: dict, pages: list[dict], doc: dict | None = None) -> list[str]:
    ...
    A page list says what exists. It does not say that Checkout is reached
    from Cart and returns to Order Tracking, so each page gets built as an
    island and the nav is whatever the model felt like on the turn it wrote
    the layout — which is how an app ends up with a page nobody can reach and
    a nav link to a route that was never written.

    The journeys are the plan's own `workflows`, turned back into routes by
    matching each step against the page names.
    """
    if not pages:
        return []
    ...
    for flow in (plan.get("workflows") or []):
        ...
        for step in steps:
            ...
            if routes:
                ...
                # A step with no page of its own — "System.
