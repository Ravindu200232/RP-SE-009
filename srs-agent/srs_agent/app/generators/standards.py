...
The published requirements-engineering baseline used by AgentForge is
ISO/IEC/IEEE 29148:2018. UML diagrams follow OMG UML 2.5.1 notation where UML
applies, BPMN follows OMG BPMN 2.0.2, and ERD/DFD use the conventional
Crow's-Foot / Yourdon-DeMarco business-analysis notations.

This module does not claim third-party certification.  It supplies a stable
information model, requirement-quality checks, verification metadata and
traceability fields so the PDF can be produced in an internationally
recognisable SRS form without silently inventing missing facts.
"""
...
def _source_map(doc: dict) -> dict[str, str]:
    """Best-effort provenance from approved plan features / screens without invention."""
...
def apply_international_profile(srs: dict) -> dict:
    ...
    Existing requirements, IDs and scope are preserved.  Only metadata used for
    document control, verification and quality review is added.
    """
