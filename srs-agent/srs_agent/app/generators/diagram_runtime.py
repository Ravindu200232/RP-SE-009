...
def diagram_applicability(kind: str, doc: dict) -> tuple[bool, str]:
    ...
    Professional requirements documents should say *not applicable* rather than
    draw a fake lifecycle/process merely to fill an appendix slot.
    """
    if kind == "state_machine":
...
def render_diagrams(project_id: str, diagrams: list[dict], *, doc: dict | None = None,
    ...
    For the complete diagram pack, the canonical output is a native ReportLab vector
    drawing exported to SVG.  This avoids pretending Mermaid flowcharts are
    literal UML/BPMN notation. Mermaid remains a portable/editable source and
    a fallback for supplemental architecture views.
    """
