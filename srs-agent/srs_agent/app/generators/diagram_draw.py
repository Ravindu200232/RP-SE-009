...
def _use_case(doc, W):
    """Actors and the things they do, from the same source as the Mermaid.

    This used to list `main_modules` as the use cases and wire actor *i* to
    module *i mod n* — an arbitrary pairing that said nothing true. With no
    renderer installed this drawing is what the PDF shows, so it has to agree
    with what the browser draws.
    """
    from .diagrams import actors_and_use_cases

    actors, use_cases = actors_and_use_cases(doc)
    actors, use_cases = actors[:6], use_cases[:9]
    kept = {u["id"] for u in use_cases}

    H = max(len(actors), len(use_cases)) * 34 + 30
    d = Drawing(W, H)
    g = Group()
    aw, ah = 95, 24
    ...
    for i, uc in enumerate(use_cases):
        cy = H - 40 - i * ((H - 60) / max(len(use_cases), 1))
        cx = sys_x + sys_w / 2
        _node(g, cx, cy, sys_w - 40, 20, uc["label"], colors.white, PRIMARY, INK, 7.5)
        centers[uc["id"]] = (cx, cy)

    for i, a in enumerate(actors):
        cy = H - 30 - i * ((H - 40) / max(len(actors), 1))
        _node(g, 12 + aw / 2, cy, aw, ah, a["label"], INDIGO_BG, PRIMARY, INK, 7.5)

        for uid in [u for u in a["does"] if u in kept][:4]:
            _arrow(g, 12 + aw, cy, sys_x, centers[uid][1], LINE)
    d.add(g)
    return d
