...
from reportlab.graphics.shapes import Circle, Drawing, Ellipse, Group, Line, Path, Polygon, Rect, String
...
def _port(g: Group, x: float, y: float, *, color=LINE_DARK, r: float = 2.6):
    """Visible connection port so relationships never look detached."""
    g.add(Circle(x, y, r, fillColor=WHITE, strokeColor=color, strokeWidth=.9))


def _curve_arrow(g: Group, x1: float, y1: float, x2: float, y2: float, *,
                 color=LINE_DARK, sw=.95, dashed=False, label: str | None = None,
                 open_head=True, label_size=6.8, bend: float = .42, ports=True):
    """Smooth routed connector with explicit ports and an attached arrow head.

    Context/component/deployment diagrams used to draw long straight diagonals.
    Those lines cut through neighbouring boxes and sometimes ended a few pixels
    away from the target, which read as an unconnected diagram in the PDF.
    Cubic routing keeps the first/last segment normal to the box edge while
    still avoiding the rigid 'only straight lines' look.
    """
    dx, dy = x2 - x1, y2 - y1
    p = Path()
    p.moveTo(x1, y1)
    if abs(dx) >= abs(dy):
        c = max(22.0, abs(dx) * bend)
        c1x = x1 + (c if dx >= 0 else -c)
        c2x = x2 - (c if dx >= 0 else -c)
        p.curveTo(c1x, y1, c2x, y2, x2, y2)
        ahx, ahy = c2x, y2
    else:
        c = max(22.0, abs(dy) * bend)
        c1y = y1 + (c if dy >= 0 else -c)
        c2y = y2 - (c if dy >= 0 else -c)
        p.curveTo(x1, c1y, x2, c2y, x2, y2)
        ahx, ahy = x2, c2y
    p.strokeColor = color
    p.strokeWidth = sw
    p.fillColor = None
    if dashed:
        p.strokeDashArray = [5, 3]
    g.add(p)
    _arrow_head(g, ahx, ahy, x2, y2, color=color, sw=sw, open_head=open_head, size=6.2)
    if ports:
        _port(g, x1, y1, color=color)
        _port(g, x2, y2, color=color)
    if label:
        mx, my = (x1 + x2) / 2, (y1 + y2) / 2
        cap = max(70, math.hypot(dx, dy) * .58)
        _txt(g, mx, my + 6, _fit(label, F, label_size, cap), label_size, F, MUTED, "middle")
