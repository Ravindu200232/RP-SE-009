...
The editable Mermaid sources are useful for portability, but the canonical
web/PDF rendering is deliberately drawn here with deterministic vector
primitives.  This keeps layout, typography and notation stable and avoids the
"auto-layout demo" appearance produced by generic flowchart engines.

Design goals:
* standards-aware symbols (UML 2.5.1 / BPMN 2.0.2 where applicable);
* readable at A4 width and at 720px web preview width;
* generous whitespace, consistent 8/12/16pt rhythm and orthogonal routing;
* no invented relationships, transitions, gateways, infrastructure or data;
* unsupported diagrams render as a professional "not applicable" panel rather
  than manufacturing semantics.
"""
