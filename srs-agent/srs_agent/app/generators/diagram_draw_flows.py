...
def _class_object(doc: dict, W: float) -> Drawing:
    ...
    class_sizes={str(t["table_name"]):_class_box for t in tables}
...
def _bpmn(doc: dict, W: float) -> Drawing:
    ...
    for i,lane in enumerate(lanes):
        ...
        # horizontal lane labels are more readable in generated PDFs than.
...
def _component(doc: dict, W: float) -> Drawing:
    ...
    def place(items,y,kind):
        count=max(1,len(items)); gap=12; left=28; avail=W-56; bw=min(142,(avail-gap*(count-1))/count); total=bw*count+gap*(count-1); start=(W-total)/2; pos=[]
