...
def snapshot_diagrams(project_id: str, version: str,
    ...
    The renderer writes to fixed names — `diagrams/{kind}.{mmd,svg,png}` — and
    `repo.save_diagrams` deletes every row before re-inserting, so a second
    revision destroys the first one's pictures with nothing to recover them
    from. The version rows in `srs_versions` hold a full snapshot of the
    document, so the history is real; only the images were being lost, which
    made "show me version 1.0.0" show version 1.2.0's diagrams.

    Copies into `diagrams/v{version}/` and returns the diagram dicts with their
    paths rewritten to the copies. The flat directory keeps holding the current
    version, so `adopt_srs`, the PDF renderer and `read_srs_results` are
    untouched — they all read the live one and should.

    `.mmd` and `.svg` only. The PNGs render at `-s 2`, are by far the largest
    artefact, and are read solely by the PDF, which is generated on demand for
