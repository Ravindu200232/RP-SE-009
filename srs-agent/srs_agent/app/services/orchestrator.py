...
def _fallback_analysis(state: dict) -> dict:
    ...
    The interview does not need the graph to produce questions — the backbone is
    deterministic — so this only has to re-derive the classification.
    """
...
def _remember(session: dict, question: dict | None) -> dict | None:
    ...
    Caching it matters: without it, remounting the interview screen would fire a
    fresh LLM call and the customer would see the same question reworded.
    """
...
def _apply_clarification(session: dict, st: dict) -> None:
    ...
    The confirmed text is what the plan and the SRS read; `raw` stays on the
    entry as the record of what the customer actually wrote.
    """
    resolved = clarify.resolve(st)
    if not resolved:
        return
...
async def _sources_by_id(project_id: str, ids: list[str] | None) -> list[dict]:
    ...
    An id that belongs to another project — or to nothing — is dropped rather
    than raised on: a stale chip must not cost them the answer they just gave.
    """
...
async def generate_plan(project_id: str, revision: str = "") -> dict:
    ...
    app_name = (session.get("answers", {}).get("app_name", {}) or {}).get("value")
    markdown = plan_generator.render_plan_markdown(
        plan, app_name=str(app_name or project.get("title", "")))
