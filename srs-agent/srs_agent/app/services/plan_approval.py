...
def _fresh(doc: dict | None, project: dict | None) -> tuple[dict | None, str]:
    """The stored plan with its journeys normalised, and its Markdown
    rendered NOW rather than read back from approval time.

    The markdown was stored once, when the plan was generated, and served
    from then on — so a plan approved a week ago kept the renderer of a week
    ago: no "User journeys" heading, no role on a journey, no owner's
    journey, however the renderer had moved on. Everything that reads plan.md
    (the review screen, the design demo, the build handoff, the E2E flows)
    saw the old shape. Rendering from the plan JSON on read costs nothing and
    means one renderer, the current one, for every plan. `content_hash` and
    approval are untouched — the plan's facts are the same; only their
