...
Three compiled graphs compose the agent nodes:

* analysis      : intake → (clarify | classify)
...
(answering questions / approving the plan), which keeps every graph a clean DAG.

The interview itself is deliberately NOT a graph node: it is one question at a
time, driven by the customer, so it lives in `agents/interview.py` and is
stepped by the orchestrator rather than run to completion.
"""
