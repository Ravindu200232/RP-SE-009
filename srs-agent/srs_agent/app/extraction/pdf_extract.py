...
# Pages are read a few at a time, not all at once. The model behind this is
# usually one local Ollama, and twelve concurrent vision calls at it is how a
# working upload becomes a timeout.
VISION_AT_ONCE = 3
...
async def read_pdf(data: bytes, filename: str = "upload.pdf") -> dict[str, Any]:
    ...
    # A page the text layer could not read. With no text engine at all there
    # are no page texts, so every page qualifies.
    ...
    for i in range(total):
        ...
        if read:
            ...
            # Neither reader got anything. Recorded, because a page that
            # silently disappears from a specification is the failure this
