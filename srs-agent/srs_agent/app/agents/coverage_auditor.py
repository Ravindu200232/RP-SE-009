...
Maintains the required coverage checklist and computes which areas the finished
interview actually closed (by the brief or by an explicit answer). Gaps are not
re-asked — the interview backbone re-derives its own queue after every answer,
so anything still missing here is something no question in the profile covers,
and it falls through to a sensible default recorded as an assumption.
"""
