...
When someone types their own answer instead of picking an option, that answer is
the part of the interview most likely to be wrong later: it is not in any option
list, nothing downstream recognises it, and a two-word note like "layaway" or
"split bill" can become a page, a table and a requirement on the strength of a
guess.

So a typed answer that is not self-explanatory goes through a short loop:

    interpret → confirm the meaning → ask the purpose → confirm the purpose

and only then is it a requirement. Two rules hold the loop together:

* **The raw text is never overwritten.** Every state keeps `raw` exactly as it
  was typed. An interpretation is an addition, never a replacement, so a wrong
  guess can always be walked back to what the customer really wrote.
* **Nothing becomes a requirement until both halves are confirmed.** `resolve()`
  returns nothing until `meaning_confirmed` *and* `purpose_confirmed` — a
  half-understood answer is not a requirement.

This module asks questions and returns JSON. It never edits the SRS and never
decides anything by itself: every transition is something the customer chose.
"""
...
def is_self_explanatory(text: str, options=None) -> bool:
    ...
    A deterministic pre-filter, on purpose. An LLM call per typed answer would
    add a slow round-trip to answers that were never ambiguous — "Customer
    loyalty points" needs no interpretation — and every avoided call is an
    avoided chance to mangle something that was already right.
    """
...
async def start(question_id: str, raw: str, *, topic: str = "", question: str = "",
    ...
    Returns a state dict. A self-explanatory answer comes back already
    `confirmed`, so the caller can run every typed answer through this without
    branching first.
    """
    state = {
...
def _clean_answer(value, question_id: str, raw: str = "") -> str:
    ...
    The model is asked for a `machine_value` and tends to qualify it with the
    topic — `app_name: School Library` — which is readable and is not the
    answer. Strip that prefix when it is this question's own key, and the
    surrounding quotes a label picks up.

    The same mistake arrives with a different separator. A real run produced
    `app_name_spoke_and_chain`, which only checked for a colon sailed through,
    and the shop was then called "app_name_spoke_and_chain" — in the plan
    heading, in the project folder, and in the browser tab of the built app.

    Stripping that leaves `spoke_and_chain`, which is still not a name. A
    machine value is a slug of something, and when the customer's own words are
    in hand they are the better answer: they typed "Spoke & Chain".
    """
    ...
    # Only when the model qualified it do we know this is a restatement rather
    # than a value in its own right — an enum answer like `email_password` must
    # survive untouched.
...
async def _retry_meaning(state, typed, existing, project_id, context=""):
    """They want to say it differently. The original text still stands.

    The attempt counter is owned by `_interpret_into` alone — counting here as
    well made a single failed reading look like two, so the customer hit the
    three-attempt ceiling after two tries.
    """
    if not typed:
        return state
