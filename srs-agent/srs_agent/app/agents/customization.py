...
LLM path: ask the model for a *patch* — only the top-level sections it changed
— and overlay that on the current document. Asking for the whole SRS back does
not work: the document is far larger than the prompt budget, so the model only
ever sees the first slice of it and any section it never saw came back missing.
Patch-and-merge makes a truncated view harmless — what the model cannot see, it
cannot drop.

Deterministic fallback: intent-detect common edits (add feature/requirement,
rename module, stricter performance, add language, add payment/approval) and
mutate only the relevant sections — unrelated content is preserved and version
history is kept by the caller.
"""
...
def _merge_list(new: list, current: list, removing: bool) -> list:
    ...
    A model told to return "only the sections you changed, each one complete"
    frequently returns the section holding only the entries it touched. That is
    indistinguishable, by length alone, from an echo of the truncated copy it
    was shown — and both used to be handled by dropping the patch entirely,
    which threw the edit away.

    Folding by identity handles both honestly: an entry the current document
    already has is replaced in place (an edit), one it does not have is
    appended (an addition), and one the patch omits is kept (never a deletion).
    Only a request that actually asked for less may end up with less.
    """
    ...
    for item, ident in zip(new, idents):
        if ident in where:
            ...
            if _is_a_different_thing(item, out[at]):
                # A NUMBER IS A SLOT, NOT A NAME. `users` and `Booking Portal`
                # identify a thing, so an entry arriving under one of those is
                # an edit of it however much its content changed. `FR-026` is a
                # position in a list, and a model asked to add a requirement
                # reaches for the next number it can think of — which is often
                # one already taken. Replacing then deletes an unrelated
                # requirement and reports it as an addition.
                #
                # Measured on a real revision: "add a Warranty Claims page"
                # came back as FR-026 and overwrote "The system shall let a
                # user edit and remove a sale record". The count stayed at 33,
                # so the never-shrink guard above saw nothing wrong.
...
def _is_a_different_thing(incoming, existing) -> bool:
    ...
    Only asked about entries keyed by a bare number, where identity carries no
    meaning. A rewrite keeps most of its words — "let a user edit a sale" to
    "let the Owner edit a sale" — so overlap stays high. A requirement about
    re-opening repairs has nothing in common with one about sale records.
    """
...
def _clean_patch(patch: dict | None, doc: dict | None = None, prompt: str = "") -> dict:
    ...
    A model that ignores the patch contract and echoes the whole document
    echoes the *truncated* copy it was shown, so a section can come back
    holding a third of its entries. Losing 19 of 22 requirements to an "add a
    loyalty programme" edit is the same bug as losing all of them.

    The length check used to compare only TOP-LEVEL lists, which left the one
    section that is a dict — `database_design` — completely unguarded. Measured
    on a real bakery SRS: an "add a waste log" revision returned
    `database_design` holding three tables, the shallow update replaced all
    seven, and `users`, `roles`, `baked_goods`, `orders` and `order_lines`
    were gone. The diff summary said only that a table had been added, so
    nothing on screen suggested the data model had just been destroyed. Dict
    sections are now walked one level and merged key by key.
    """
    doc = doc or {}
