...
def _open_question(raw) -> dict:
    ...
    `options` used to be dropped here, so a question always arrived at the
    review screen as bare text with a box under it — and an either/or question
    with nothing to click gets answered "yes", which settles nothing and comes
...
def _journeys_for_every_role(plan: dict) -> list[dict]:
    """The plan's workflows as user journeys: each with its `who`, and one
    for every role in `users`.

    The model is asked for a journey per role, and it usually gives the
    obvious one — the customer buying — and stops. That single journey then
    becomes the handoff's only road map and the E2E's only flow, and the
    owner's half of the app is never walked by anyone before the customer.
    So: a journey with no `who` is given the role its steps name (or the
    first role); a role with no journey gets one written from its own
    `can_do` lines, plainly labelled as that role's day-to-day. Nothing the
    customer said is invented — the steps are their words from `users`.
    """
