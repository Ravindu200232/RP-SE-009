...
The studio reaches the SRS through Next's rewrite, and that rewrite gives up on
a request after 30 seconds. Measured, not assumed: `generate-srs` for a small
school library returned 200 in 84.9s straight to Python and 500 at exactly
30.0s through the rewrite. The standalone SRS app has the same problem and
solves it by bypassing its own proxy, which is not available here — the studio
and the app share an origin on purpose, so the preview iframe stays reachable.

So every slow call becomes three fast ones: start a job, poll it, read the
result. That is also how AgentForge already handles its own long POSTs.

This is deliberately generic rather than one shim per slow endpoint. The 30s
cliff is not specific to `generate-srs`: a single interview answer was measured
at 23.3s on a fast cloud model, and every LLM call in here scales with whichever
model the user picked. An endpoint-by-endpoint list would be a list of the ones
that happened to be slow on this machine, on this day.

Requests are replayed into the app in-process over ASGI — no socket, no second
port, and the eight copied routers do not learn that any of this exists.
"""
