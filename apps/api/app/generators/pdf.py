"""Professional SRS PDF generator (ReportLab).

Renders a StayEase-style IEEE-830 report: cover page, table of contents with
page numbers, sections 1–9, styled tables, status badges, page numbers, and an
appendix that embeds rendered diagrams (PNG) — falling back to the Mermaid
source in a code box when a diagram wasn't rendered. ReportLab is pure-Python,
so this works on Windows without native libraries.
"""
...
class _Doc(BaseDocTemplate):
    ...
    def _blank(self, canvas, doc):  # cover page: no chrome
        pass

    def _chrome(self, canvas, doc):
        canvas.saveState()
        # Header rule + project name
        ...
        canvas.drawString(self.leftMargin, 1.1 * cm, "AgentForge Studio · IEEE 830")
...
def build_srs_pdf(srs: dict, out_path: str | Path, *, status: str = "Draft",
    ...
    def h1(n, text):
        story.append(Paragraph(f"{n}. {text}", st["h1"]))
    ...
    body("SRS — Software Requirements Specification; FR — Functional Requirement; "
         "NFR — Non-Functional Requirement; RBAC — Role-Based Access Control; "
         "RTM — Requirement Traceability Matrix; PWA — Progressive Web App.")
    h2("1.4 References")
    body("IEEE Std 830-1998 Recommended Practice for Software Requirements Specifications.")
    h2("1.5 Overview")
    body("Section 2 gives an overall description; Section 3 specifies detailed requirements; "
         "Sections 4–9 cover data design, access control, UI/UX, risks, acceptance criteria, and diagrams.")

    # 2. Overall Description
    h1(2, "Overall Description")
    h2("2.1 Product Perspective")
    body(f"The product is a {(doc_data.get('app_type', {}) or {}).get('primary_type', 'web application')}.")
    ...
    frows = [[_P("ID", st, "cellb"), _P("Module", st, "cellb"), _P("Requirement", st, "cellb"), _P("Priority", st, "cellb")]]
    ...
    story.append(_table(frows, [1.7 * cm, 3.2 * cm, W - 7.4 * cm, 2.5 * cm], st))

    h2("3.2 Non-Functional Requirements")
    nrows = [[_P("ID", st, "cellb"), _P("Category", st, "cellb"), _P("Requirement", st, "cellb")]]
    for nfr in doc_data.get("non_functional_requirements", []):
        nrows.append([_P(nfr.get("id"), st, "cellb"), _P(nfr.get("category"), st), _P(nfr.get("requirement"), st)])
    story.append(_table(nrows, [1.7 * cm, 3 * cm, W - 4.7 * cm], st))

    h2("3.3 Security Requirements")
    for sreq in doc_data.get("security_requirements", []):
        story.append(_P(f"• {sreq}", st, "body"))

    h2("3.4 External Interface & Integration Requirements")
    ...
    h2("3.6 Requirement Traceability Matrix")
    trows = [[_P("Req", st, "cellb"), _P("Module", st, "cellb"), _P("Tables", st, "cellb"), _P("Test", st, "cellb")]]
    for tr in doc_data.get("requirement_traceability_matrix", [])[:60]:
        trows.append([_P(tr.get("requirement_id"), st, "cellb"), _P(tr.get("module"), st),
                      _P(", ".join(tr.get("tables", [])), st), _P(tr.get("test_case"), st)])
    story.append(_table(trows, [2 * cm, 4 * cm, W - 9 * cm, 3 * cm], st))
    ...
    # 7. Risks and Priorities
    story.append(PageBreak())
    h1(7, "Risks and Priorities")
    ...
    # 9. Appendix — Diagrams
    story.append(PageBreak())
    h1(9, "Appendix — Diagrams")
    diagrams = diagrams if diagrams is not None else doc_data.get("diagrams", [])
    story.append(_P(f"This appendix contains all {len(diagrams)} generated diagrams, drawn "
                    "from the specification. (A higher-fidelity Mermaid render is embedded "
                    "instead when mermaid-cli is installed; the web app also renders them live.)",
                    st, "small"))
    max_img_h = pdf.height - 4 * cm
    for idx, d in enumerate(diagrams):
        if idx > 0:
            story.append(PageBreak())
        title = d.get("title", d.get("kind", "Diagram"))
        by = d.get("generated_by")
        story.append(Paragraph(f"9.{idx + 1} {title}", st["h2"]))
        if by:
            story.append(_P(f"Source model: {'LLM-generated' if by == 'llm' else 'template'} Mermaid · "
                            "drawn natively for print", st, "small"))
        story.append(Spacer(1, 6))

        placed = False
        # 1) High-fidelity mermaid render if available.
        png = d.get("png_path")
        if png and Path(png).exists():
            try:
                ...
                placed = False
        # 2) Native vector drawing from the SRS data (always works, offline).
        if not placed:
            drawing = render_native(d.get("kind", ""), doc_data, W)
            if drawing is not None:
                if drawing.height > max_img_h:  # scale tall diagrams to fit the page
                    s = max_img_h / drawing.height
                    drawing.scale(s, s)
                    drawing.width *= s
                    drawing.height *= s
                story.append(drawing)
                placed = True
        # 3) Last resort: readable source box.
        if not placed:
