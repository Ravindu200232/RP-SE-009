...
Builds a complete, schema-valid, domain-specific SRS from the domain template +
the user's answers, with no LLM call. Used as the offline fallback and as the
seed/skeleton the LLM enriches. Output matches the canonical root schema.
"""
...
def _project_name(title: str, domain: dict) -> str:
    ...
    # Title-case short ideas, otherwise keep as-is.
...
def build_offline_srs(
    *,
    ...
    # ── Role access matrix ───────────────────────────────────
    all_page_names = [p["page_name"] for p in public_pages] + [p["page_name"] for p in protected_pages]
    role_access = []
    for r in roles:
        rk = r["role_key"]
    ...
    # ── API design (CRUD per domain table) ───────────────────
    api_design = [
    ...
    # ── Traceability matrix ──────────────────────────────────
    page_by_module = {p.get("page_type", ""): p["page_name"] for p in protected_pages}
    rtm = []
    for fr in frs:
...
# ─────────────────────────────────────────────────────────────
#  LLM "enrichment pack" merge
#
#  Rather than asking the LLM to regenerate the entire ~1500-line SRS (slow and
#  fragile for a reasoning model), we ask it for focused domain CONTENT (tables,
#  modules, requirements, workflows…) and assemble a guaranteed-valid SRS around
#  it here. The structure is fully controlled, so the result always validates.
# ─────────────────────────────────────────────────────────────
...
def _derive_role_matrix(roles: list[dict], public_pages: list[dict], protected_pages: list[dict]) -> list[dict]:
    ...
    for r in roles:
        ...
        if rk == "super_admin":
            ...
            if not pages:  # role not referenced by any page → give a sensible default
...
def merge_pack(skeleton: dict, pack: dict) -> dict:
    """Overlay an LLM enrichment pack onto the deterministic skeleton."""
    ...
    # functional requirements: universal core + pack
    frs: list[dict] = []
    ...
    # non-functional: skeleton + pack, re-id
    ...
    # regenerate cross-cutting sections from the merged content (keep consistent)
