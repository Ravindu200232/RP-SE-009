...
Every emitted event is (a) written to the ``agent_events`` collection and
(b) pushed to any live SSE subscribers for that project. Channels map to the
five tabs of the analyzer console bar: live_logs, agent_events, terminal,
prompt_trace, errors.
"""
...
class EventBus:
    ...
    async def log(self, project_id: str, agent: str, message: str, **kw) -> dict:
        """Convenience: emit to the live_logs view (channel overridable)."""
    ...
    async def subscribe(self, project_id: str) -> AsyncIterator[dict]:
        ...
        try:
            # Replay recent history so a late subscriber sees prior progress.
