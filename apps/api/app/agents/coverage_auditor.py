"""CoverageAuditorAgent — score SRS coverage and decide on follow-ups.

Maintains the required coverage checklist. After each answer batch it computes
which areas are now covered (by the brief or an explicit answer), keeps asking
only for missing *critical* areas, and lets optional gaps fall through to
sensible defaults (recorded later as assumptions/ambiguities).
"""
from __future__ import annotations

from ..schemas.questions import COVERAGE_AREAS, CRITICAL_AREAS
from ..services.events import bus
from .question_planner import _bank, _covered
...
def compute_coverage(brief: str, questions: list[dict], answers: list[dict]) -> dict:
    ...
    # Optional gaps count partially (they become defaulted assumptions).
...
def build_followups(domain_key: str, areas: list[str], start_index: int) -> list[dict]:
    bank = _bank(domain_key)
    out = []
    for i, area in enumerate(areas, start=start_index):
        if area not in bank:
            continue
        q = bank[area]
        out.append({
