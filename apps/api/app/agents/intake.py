...
Reads the merged brief (idea + PDF/OCR/voice text), detects language, and
decides whether the input is too meaningless to build an SRS from. If so the
flow routes to clarification instead of fabricating a fake SRS.
"""
...
# Function + business words. A genuine idea almost always contains a few of
# these; keyboard-mash ("asdasdas") contains none — that is the main signal.
_COMMON = {
...
# Letter clusters that essentially never appear in English words but are
# typical of home/edge keyboard rows.
...
def _plausible_word(w: str) -> bool:
    ...
    if any(m in w for m in _MASH):
        return False
    # repeated short seed: "asdasdas" = "asd"*2 + "as"
    if len(w) >= 6 and w.count(w[:3]) >= 2:
        return False
    if re.search(r"(.)\1\1", w):  # 3+ identical letters in a row
        return False
...
def looks_like_nonsense(text: str) -> tuple[bool, str]:
    ...
    # No anchor words AND mostly implausible tokens → mash.
