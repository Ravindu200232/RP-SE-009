...
- Every `data_model.collection` must participate in at least one real planned page/API route; do not plan seed-only entities that never appear in the user journey.
- Every route `reads`/`writes` name must exactly match a `data_model.collection`. Singular/plural aliases are different collections and are not interchangeable.
- Every relationship must name an existing source field and an existing target collection, and the same identifier representation must be used in seed, links, API params, Mongo queries, responses, and UI state.
- For auth apps, `lib/auth.js` owns provider account creation. `lib/seed.js` may call only `ensureDemoAccounts()` for identity provisioning, then seed product data. Never plan Better Auth setup, `auth.api.signUpEmail`, password hashing, or direct credential collection inserts inside `lib/seed.js`.
- A route is not complete because its file exists: its visible control, request method/path, handler, persistence effect, response shape, navigation result, and E2E proof must agree exactly.
- Plan every route so a first load with valid seed data can render without a 404/500, missing relation, undefined field, invalid ObjectId conversion, or impossible auth precondition.
