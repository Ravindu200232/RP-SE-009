...
class AnalyzerRepairApplyMixin:
    def _evidence_still_open(self, findings, proposed: dict, files: dict) -> list:
        ...
        Runs the same grep the detectors run, on disk-content overlaid with
        the candidate writes. Deterministic and cheap — no model, no disk.
        """
        overlay = dict(files)
    ...
    def repair(self, report: AnalyzerReport, server_log: str = "") -> int:
        ...
        # One-hop local UI components are part of the same repair surface.
        ...
        # Prove the write closes its finding BEFORE it reaches disk. A round
        # that ships an unverified rewrite pays for it later with a rescan, a
        # rebuild and the same finding again — measured at four such laps on
        # one build, all chasing the same two testimonials findings.
