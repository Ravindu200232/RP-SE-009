...
class AnalyzerRuntimeMixin:
    ...
    def probe_pages(self, report: AnalyzerReport, *, skip_root: bool = False) -> None:
        ...
        V16 mixed pages and API routes into the same generic verification pass
        and then repeated that pass again after E2E/security.  V17 gives every
        surface one owner: runtime owns pages, API verification owns GET APIs,
        and E2E owns business mutations.
        """
    ...
    def probe_api_routes(self, report: AnalyzerReport, *, skip_health: bool = False) -> None:
        ...
        401/403 are valid for protected APIs because this probe is deliberately
        unauthenticated.  Mutating methods are not fired here; their real
        persistence/authorization behavior belongs to the accepted E2E
        journeys.  Only 404/5xx are API probe failures.
        """
        for url, r in sorted(report.routes.items()):
    ...
    def probe_routes(self, report: AnalyzerReport) -> None:
        ...
        The main build pipeline no longer calls this combined method.
        """
    ...
    def probe_linked_dynamic_routes(self, report: AnalyzerReport, limit: int = 24) -> None:
        ...
        Merely having `app/rooms/[id]/page.jsx` says nothing about whether
        `/rooms/<a real seeded id>` works. The hotel build proved the gap: the
        file existed and every static route was 200, but it queried Mongo with
        `{_id: id}` where `id` was a URL string, so every room card led to
        Next's 404. When an SSR list page already contains concrete hrefs,
        those are free runtime test cases — follow them.

        Only 404/5xx count here. 401/403 can be correct for a link whose page
        needs a session; authenticated journeys are handled by E2E.
        """
