...
class BugFixerScopeMixin:
    def __init__(self, arch, project_dir=None, *, callbacks=None,
        ...
        self.refusals = {}
        # Test files whose prompt hid passing-case bodies; their commits
        # must go through the splice that puts those bodies back.
        self._collapsed = set()
    ...
    def runtime_editable(self, rel, privileged_paths=None) -> bool:
        ...
        Runtime evidence outranks scaffold provenance. If a diagnosed failure
        actually leads to auth, Mongo wiring, a shared lib, API route, page or
        component, that source must be repairable. We still reject paths outside
        the generated project and unsafe metadata/dependency paths; that is path
        safety, not a change-size or architecture gate.
        """
    ...
    def missing_export(self, test_file, target) -> set:
        ...
        Wrong by `agents/core/exports_parse.py + agents/core/exports_checks.py` — the same parser that found six real
