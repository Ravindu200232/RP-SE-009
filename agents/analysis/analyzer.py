    ...
    def _data_ui_invariants(self):
        ...
        for rel, body in sorted(self.code_files().items()):
            ...
            if rel.startswith(("app/", "components/")):
                ...
                if mutates and not re.search(r"\btoast(?:\.|\()|set(?:Error|Success|Message|Notice|Feedback)\s*\(|aria-live|role\s*=\s*['\"]alert", body): out.append(Finding("major", "MUTATION_FEEDBACK_MISSING", "auth/mutation UI gives no visible React success/error feedback", rel, "use the shared non-blocking React toast/inline feedback after the real result", [rel]))
