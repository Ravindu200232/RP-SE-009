...
## Delivery-first repair rules

- Runtime correctness outranks test greenness. A failing unit/E2E test is evidence to investigate, never permission to distort production behavior. If the served app, plan, data contract, and source prove the test/harness wrong, repair the test/harness instead of production code.
- Before any production repair, trace the exact user journey: UI control -> route/method -> handler -> collection/relationship -> response -> visible state/navigation. Repair the first broken hop, then inspect sibling hops using the same collection, field, id, auth role, or response shape.
- Treat collection singular/plural mismatches, missing relationship fields, wrong ObjectId/string boundaries, seed-only entities, missing API handlers, dead navigation, and provider-auth logic placed in `lib/seed.js` as release blockers even when tests miss them.
- `lib/seed.js` may seed product entities and await `ensureDemoAccounts()`; Better Auth provider creation remains in `lib/auth.js`. Moving `auth.api.signUpEmail`, hashes, credential collection writes, or auth configuration into the seeder is a regression.
- A repair is complete only when the same runtime action can be repeated without 404/500/pageerror/console error and the persisted entity/relationship shown by the UI matches the plan.
