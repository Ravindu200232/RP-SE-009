...
class BugFixerPromptMixin:
    ...
    def _case_blocks(cls, body: str):
        ...
        A small paren scanner that knows about strings, template literals
        and comments. None means the file did not scan cleanly, and every
        caller must fall back to whole-file handling — never guess.
        """
        body = body or ""
        out = []
        for m in cls._CASE_START_RE.finditer(body):
            ...
            # scanning from after the name found the arrow's paren instead
            # and closed the block thirteen characters in.
            i = body.index("(", start)
            depth, n = 0, len(body)
            in_str, esc = "", False
            while i < n:
    ...
    def _splice_passing(cls, old: str, new: str, failing) -> str:
        ...
        The measured worst failure mode here is the wholesale rewrite: five
        cases, one red, and the "fix" comes back with three red. Restoring
        the four untouched cases byte-for-byte makes that outcome impossible
        whatever the model wrote. Returns "" when the two files do not line
        up cleanly, and the caller keeps the model's text.
        """
    ...
    def _prompt(self, failures, test_body, target_body, forced, reason,
        ...
        # Hide passing bodies when the splice can restore them: half the
        # tokens on a big file, and the model physically cannot break a case
        # it never saw the body of.
