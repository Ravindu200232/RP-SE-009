"""Project inventory, plan comparison, route enumeration and contract topology checks."""
...
class AnalyzerContextMixin:
    ...
    def unreachable_pages(self, routes: dict = None) -> list:
        ...
        `dead_links` catches a link to a page nobody wrote. This is the other
        half, and it is the one people meet: pages written, compiling,
        serving — and no way to reach them.

        Measured on a pharmacy build: fifteen routes, every one green, and
        the home page rendered four category filters, a search box and
        nothing else. No header, no nav; `components/` held eleven components
        and not one was navigation; `app/layout.jsx` rendered `{children}`
        alone. The basket, the checkout, the orders, the sign-in and every
        staff page were unreachable. Build clean, browser tests passed,
        verification found one major — nothing was looking for this.

        It is a walk from `/`, not a set difference. Counting "is this route
        mentioned anywhere" calls a page reachable when the only thing
        linking to it is itself unreachable — on that same build it found two
        orphans instead of fourteen, because /checkout is linked from
        /basket and /basket is linked from nowhere.

        The shell is part of every page: whatever `app/layout.*` links to is
        reachable from everywhere, which is exactly what a navbar is for.
        Dynamic routes are skipped — they are reached from a list, with an
        href built at runtime from a record's id.
        """
        routes = self.enumerate_routes() if routes is None else routes
    ...
    def contract_findings(self, routes: dict = None) -> list:
        ...
        The planner now records the exact caller, runtime target and HTTP method
        for actions that cross a file boundary.  That turns several previously
        semantic bugs into facts: a POST contract whose handler exports only GET
        is broken, and a static target never named by the caller/import closure
        has not been wired. Dynamic URLs stay with the semantic/browser audit
        because `/items/[id]` normally appears in code as `/items/${id}`.
        """
