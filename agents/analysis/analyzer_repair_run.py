"""Analyzer repair orchestration and runtime pass."""
...
class AnalyzerRepairRunMixin:
    def run(self, *, use_model: bool = True, max_rounds: int = 2,
        ...
        `UNBUILT_PROMISE` used to be detected as a *major* finding after the
        blocker scan, while the repair loop was guarded by `report.blockers()`.
        In other words the analyzer could correctly say "this planned feature
        is absent" and then deliberately do nothing about it.  Flow contracts
        are repairable for the same reason: a disconnected task is not done.
        """
...
from agents.analysis.analyzer_repair_findings import AnalyzerRepairFindingsMixin
from agents.analysis.analyzer_repair_apply import AnalyzerRepairApplyMixin


class AnalyzerRepairMixin(AnalyzerRepairFindingsMixin, AnalyzerRepairApplyMixin,
                          AnalyzerRepairRunMixin):
    """Concrete repair behavior composed from finding/apply/run stages."""
    pass
