...
from agents.features.feature_prompts import feature_prompt, render_feature_prompt
...
class FeaturesAgentAuditMixin:
    ...
    def _explicit_image_gap(self, request: str, spec: FeatureSpec, selected_path: str = "") -> dict | None:
        """Require a local generated-asset reference for explicit image features."""
        # Source: feature_prompts.py — imported helper(s) come from this file.
        from .feature_prompts import feature_image_requested
        # From: agents/features/feature_prompts.py
    ...
    # Check parse against the pipeline rules and report anything that is unsafe or incomplete.
    def _audit_parse(self, text: str) -> dict:
        """Prepare the audit parse value or state used by this focused pipeline step."""
    ...
    # Check source against the pipeline rules and report anything that is unsafe or incomplete.
    def _audit_source(self, paths) -> str:
        """Prepare the audit source value or state used by this focused pipeline step."""
        live = getattr(self.arch, "files", {}) or {}
    ...
    # Returns PASS or an evidence-backed delta plan; never mutate source.
