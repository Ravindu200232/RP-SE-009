...
class CommandResult:
    # Prepares CommandResult with the services and starting state it needs before it begins work.
    ...
    # Converts a command result into short model-readable feedback.
    def as_feedback(self) -> str:
        """Convert a command result into short model-readable feedback."""
        head = f"$ {self.command}\n"
...
# Returns the safe command parts, or a reason for refusing the command.
...
class CommandRunner:
    ...
    # Prepares CommandRunner with the services and starting state it needs before it begins work.
    ...
    # Finds the npx program beside npm, with the system path as fallback.
    def _npx(self) -> str:
    ...
    def run(self, command: str, timeout: int = DEFAULT_TIMEOUT) -> CommandResult:
        ...
        if _uses_package_manager(argv):
            try:
                # Source: harness_common.py — imported helper(s) come from this file.
