...
# Writes a cancellation status message without letting logging errors interrupt shutdown.
def _log_cancel_message(level: str, text: str) -> None:
    """Write a cancellation status message without letting logging errors interrupt shutdown."""
    if log is not None:
        try:
            log(level, text)
        except Exception:
...
# Starts a new cancellation scope so later work can be stopped safely by the user.
def begin() -> None:
...
# Mark the current cancellable operation as finished and remove its active cancellation state.
def finish() -> None:
...
def request() -> dict:
    ...
    for p in victims:
        _kill(p)
    _log_cancel_message("WARN", f"   ⏹ cancel requested — stopping {len(victims)} child process(es)")
    return {"ok": True, **who}
...
class track:
    ...
    # Prepares track with the services and starting state it needs before it begins work.
    def __init__(self, proc):
        """Prepare this helper with the state it needs."""
        self.proc = proc

    # Prepare this object for use inside a managed context.
    def __enter__(self):
        """Start this protected operation and return the active helper."""
    ...
    # Finishes this managed operation and release any temporary resources.
    def __exit__(self, *exc):
        """Finish this protected operation and release any temporary state."""
        with _lock:
...
def _kill(proc) -> None:
    ...
    try:
        ...
        _log_cancel_message("WARN", f"   ⚠ could not stop pid {getattr(proc, 'pid', '?')}: {e}")



# Remove the cancelled project's files and saved specification. The supplied project remover keeps all project
# cleanup rules in one place.
def cleanup(prod_dir: Path, delete_project=None) -> dict:
    ...
    if project and delete_project is not None:
        try:
            # From: agents/server/projects/project_files.py
...
# Runs a child process that can be stopped by a cancellation request.
