...
class ArchitectRuntimeMixin:
    ...
    def _builder_sys(self) -> str:
        ...
        try:
            from agents.features.source_guidance import HUMAN_COMMENT_POLICY
    ...
    def _stream(self, messages, on_delta, tools=None, temperature=0.6,
        ...
        A turn that stalls in the reasoning pass is taken again with thinking
        off rather than given up on. That is safe precisely because it stalled:
        `stalled` can only be true when not one character reached `on_delta`,
        so there is no half-written file to undo and no text to duplicate. The
        second attempt loses the reasoning pass on one turn, which is a smaller
        price than losing the turn.
        """
    ...
    def _stream_once(self, messages, on_delta, *, tools, temperature, model,
                     timeout, think):
        """One attempt at the turn. Returns `(tool_calls, stalled)`."""
...
- The approved plan/SRS look-and-feel is binding. Keep its visual language coherent instead of inventing a second design system.
...
- Reuse the content width, page padding and section spacing implied by the approved look. When it is silent, use `mx-auto w-full max-w-5xl px-4 sm:px-6` and `space-y-6`.
- A page starts with a clear header block when its job needs one. Never a bare table.

## Type

- Reuse the approved font family, sizes, weights, line heights and tracking. Generic Tailwind type sizes are fallback values only when the plan is silent for that level.
- Numbers a person compares — money, counts, dates — get `tabular-nums` without changing the approved type scale.

## Colour and depth

- Reuse the approved page/surface/text/border/accent direction consistently. Do not flatten a multi-accent direction into one accent, and add new colours only for semantic success/warning/error states.
- Reuse one coherent border-versus-shadow strategy and radius scale across the app.
...
- The submit button disables and says what it is doing ("Saving…") while the
  request is in flight, so nothing is submitted twice.
...
"""
    ...
    def _design_md(self) -> str:
        ...
        The look comes out of `plan.md` rather than `self.plan`: the
        architect's plan JSON is phases and files, and `look_and_feel` is the
        SRS plan's field — it reaches here as the `## Look and feel` section
        `render_plan_markdown` writes.
        """
