...
class ArchitectTurnMixin:
    def _task_now(self) -> str:
        ...
        The plan says it under `### Task 3 — …` and the JSON says it in
        `goal` / `done_when`; they are one decision written twice, so either
        will do and the JSON is already parsed.
        """
    ...
    FORM_RE = re.compile(r"\b(forms?|inputs?|fields?|textareas?|validation|"
                         ...
                         r"register|checkout|uploads?)\b")
    LIST_RE = re.compile(r"\b(lists?|tables?|grids?|rows?|feeds?|catalogues?|"
                         r"catalogs?|history|results|inventory|orders|"
                         r"bookings|appointments|members|records)\b")

    def _turn_reminder(self, files: list) -> str:
        """One rule from the system prompt, re-sent with the turn that is
        about to break it.

        Not a memory fix — the system prompt is `convo[0]` and `_trim_convo`
        only ever touches index 3 and up, so all of it is in front of the
        model on every turn. It is an attention fix, and the decay it answers
        is measured: across the 15 finished apps `transition-` reached every
        one of the 147 files that render a `<button>` while `focus-visible:` —
        the very next clause of the same bullet — reached 9, and of the 40
        such files written in the last third of a build, none. Thirty-four
        thousand characters read once at the top does not weigh the same at
        task five as at task one; the same sentence delivered next to the file
        it applies to does.

        Three things keep this cheap. It is a QUOTE, never a new rule, so
        there is nothing to reconcile. It is chosen by what the next files
        ARE, so a turn of route handlers is not lectured about empty states.
        And it rotates, so no line lands twice while another is still unsent —
        the value of this channel is that it varies, not that it is long. When
        nothing applies, nothing is sent.
        """
        if not files:
            return ""
