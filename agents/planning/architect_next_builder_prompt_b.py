...
PROMPT_PART_B = r"""
   ...
   USER-OWNED DATA IS ALWAYS SCOPED BY THE SERVER SESSION. If this app has
   authentication, carts, bookings, order/history rows, reviews, favourites,
   saved items, personal records and similar resources never trust a client
   `userId`, `ownerId`, email or role to decide ownership. Derive the current
   user with `getSessionUser()` inside the server page/route, convert `user.id`
   to the field's declared Mongo type exactly once at the DB boundary, and use
   that same representation for CREATE, LIST, GET, UPDATE and DELETE. An admin
   endpoint may intentionally query across owners only when the approved plan
   grants that role. If the approved plan has NO authentication, do not invent
   users, sessions or ownership merely because a record could theoretically
   belong to somebody.

   MUTATIONS MUST UPDATE THE SCREEN WITHOUT A MANUAL BROWSER REFRESH. After a
   successful POST/PUT/PATCH/DELETE, use the returned server record to update
   local state/cache immediately OR call the appropriate `router.refresh()` /
   revalidation/navigation mechanism. A successful 2xx request followed by a
   stale table/cart/history is still a broken flow. Keep one source of truth:
   do not append a guessed client object when the API returned the canonical
   saved record.
   ...
   Registration has the same contract: do not assume signup must redirect to
   `/login`. Better Auth may leave the new user signed in. Follow the approved
   product flow and the actual session result: an auto-signed-in user goes to
   their planned landing page; a flow that deliberately requires verification
   or a later login goes to that planned screen. Links, redirect code and E2E
   expectations must all agree on the same outcome.
