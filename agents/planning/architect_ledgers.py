...
class ArchitectLedgerMixin:
    ...
    def _runtime_url_of_planned(path: str) -> str:
        ...
        `_url_of_planned` intentionally covers pages only. Cross-file contracts
        also point at `/api/...`, so generation memory needs the route-handler
        half as well. Route groups never appear in the runtime URL.
        """
        p = (path or "").replace("\\", "/")
        if not p.startswith("app/"):
            return ""
    ...
    def _contract_ledger(self, wanted: list = None) -> str:
        ...
        A workflow says what the human is doing. A contract says what two files
        must agree on: runtime URL, method, request/response field vocabulary and
        the success effect. This is intentionally generated from plan.json on
        every turn instead of trusting conversational memory.
        """
    ...
    def _related_context_files(self, wanted: list) -> set:
        ...
        Folder proximity is useful for styling; it is not enough for behaviour.
        A form in `components/` may depend on an API route in `app/api/`, and a
        list page written two tasks earlier may be the only caller of a detail
        page written now. Contracts make those distant files neighbours.
        """
    ...
    def _data_ledger(self) -> str:
        """The canonical Mongo collection/field vocabulary from plan.md."""
        return self._section_of_plan("## Data Model", limit=28)

    def _flow_ledger(self) -> str:
        """The journeys, and which page leads to which.

        `## Page Flow` is the half of the plan that says how the app hangs
        together — the chains of routes a person walks, and which pages are in
        the nav against which are reached from inside another page. It is read
        once, in task one, and by task four the model is writing a page
        without the thing that says what links TO it.

        That is how a page ends up unreachable: not written wrong, written
        unconnected. Measured on a pharmacy build — fifteen routes, every one
        green, and twelve of them with nothing linking to them. On a library
        build the librarian dashboard linked two of its three children and
        forgot the third, which is the same failure one page wide.
        """
    ...
    def _design_ledger(self) -> str:
        ...
        Stated once, in task one, and every screen after that is supposed to
        match it. The continue turn asked for "one accent colour" without
        naming it, which is not an instruction anybody can follow — so the
        later tasks picked their own and the app came out in three palettes.
        """
        md = self.plan_md or ""
    ...
    def _house_ledger(self) -> str:
        ...
        Derived rather than remembered, for the same reason the symbol ledger
        is: the thread gets compacted, and what survives is whatever is put
        back. These are the things a long build drifts on — Mongo field names,
        the journeys, the palette, the roles, and which URLs actually exist.
        """
        parts = []
    ...
    def _task_note(self, index: int, task: dict) -> str:
        ...
        Cheap and deterministic — the plan and the files are both in memory,
        and nothing here costs a model call. Kept to a few lines because it
        rides a turn that already has plenty to say.
        """
