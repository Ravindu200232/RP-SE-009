...
class ArchitectDeliveryMixin:
    ...
    def delivery_gaps(self) -> list:
        ...
        Planning already refuses a half-app.  This second gate checks reality
        on disk, because a long model turn can still stop before the last page
        or silently omit one capability file.  It never invents work: every
        reported gap comes from the accepted machine plan.
        """
        gaps = []
    ...
    def close_delivery_gaps(self, max_rounds: int = 2) -> list:
        """Ask the existing builder thread to close only deterministic gaps."""
    ...
    # Module not found: Can't resolve 'date-fns' and the bare Cannot.
