...
class ArchitectScaffoldMixin:
    ...
    PRIVILEGED_ROLE = re.compile(
        ...
        r"librarian|receptionist|supervisor|accountant|hr)\b", re.I)

    def _signup_role(self) -> str:
        """The role a person gets by filling in the sign-up form.

        This used to be `demo_accounts[0]["role"]` — "the plan's own first
        role" — and planners list the administrator first, because that is
        the account they are thinking about. So the generated `lib/auth.js`
        carried `role: { defaultValue: 'admin' }`, and every app AgentForge
        built let ANY visitor create an administrator by signing up. Measured
        on a hotel build: `defaultValue: "admin"`, two demo accounts, admin
        first. It is not a cosmetic bug — it is the whole access model
        inverted, and it shipped in every app with sign-in.

        The rule now: sign-up creates the PUBLIC role — the one the app's
        customers hold — and never a privileged one. The plan may name it
        outright (`signup_role`); otherwise it is the first demo account
        whose role is not administrative. If every role is administrative,
        nobody gets one by signing up: the fallback is Better Auth's own
        powerless 'user', which no page grants anything to. An app whose
        staff are its only users seeds those accounts; it does not let the
        internet register for them.
        """
        plan = self.plan or {}
    ...
    def _keep_installed_deps(self, pkg: dict) -> None:
        """Fold anything already declared into the package.json about to be
        written, so the scaffold cannot prune what someone else installed.

        `npm install` removes packages the manifest does not name. The QA
        harness installs its toolchain — vitest, jsdom, the testing-library
        packages — with `npm install -D`, which declares them; and the
        scaffold then wrote package.json with its own pinned devDependencies
        alone, so the next install took them straight back out. Every test
        run after that died on `Cannot find package 'vitest'` and the whole
        QA stage was skipped, on a build where nothing was wrong with the
        tests.

        The scaffold's pins still win on the names it owns — the toolchain
        the config files on disk are written for — and everything else is
        kept as it stands.
        """
        try:
            on_disk = json.loads(
