...
where the stubs get filled in. Concretely, a page is finished only when:
  ...
  • every interactive control also has a stable accessible name. Icon-only
    ...
    only a glyph. Every input/select has an associated htmlFor/id label or
    an aria-label. This is product accessibility first and also lets the
    E2E author observe the real control instead of guessing selectors;
