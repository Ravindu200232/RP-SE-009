...
class ArchitectPlanNormalizeMixin:
    ...
    def _normalise_plan(self, data: dict) -> dict:
        ...
        role_keys = {str(a.get("role") or "").strip(): a for a in accounts
                     if str(a.get("role") or "").strip()}
        role_alias = {re.sub(r"[^a-z0-9]+", "", role.lower()): role
                      for role in role_keys}
        homes = {}
        raw_homes = data.get("role_homes") or {}
        if isinstance(raw_homes, dict):
            for role, route in raw_homes.items():
                key = re.sub(r"[^a-z0-9]+", "", str(role or "").lower())
                exact = role_alias.get(key, "")
                route = str(route or "").strip().split("?", 1)[0]
                if exact and route.startswith("/"):
                    homes[exact] = route.rstrip("/") or "/"

        # Recover a missing map from the machine workflows, then from ROLE
        # page purposes. This keeps older plans compatible while making the
        # login destination deterministic for every account.
        for wf in data.get("workflows") or []:
            if not isinstance(wf, dict):
                continue
            who_raw = str(wf.get("who") or "").strip()
            who = role_alias.get(re.sub(r"[^a-z0-9]+", "", who_raw.lower()), "")
            if not who or who in homes:
                continue
            for step in wf.get("steps") or []:
                m = re.match(r"\s*(/[^\s—]+)", str(step or ""))
                if m and "[" not in m.group(1):
                    homes[who] = m.group(1).split("?", 1)[0].rstrip("/") or "/"
                    break
        for ph in data.get("phases") or []:
            ...
            for f in ph.get("files") or []:
                if not isinstance(f, dict):
                    continue
                m = re.match(r"\s*ROLE\s+([^—:-]+)", str(f.get("purpose") or ""), re.I)
                if not m:
                    continue
                role_raw = str(m.group(1) or "").strip()
                role = role_alias.get(re.sub(r"[^a-z0-9]+", "", role_raw.lower()), "")
                path = str(f.get("path") or "").replace("\\", "/")
                if not role or role in homes or not re.fullmatch(r"app/(?:.+/)?page\.jsx?", path):
                    continue
                parts = [x for x in path.split("/")[1:-1]
                         if not (x.startswith("(") and x.endswith(")"))]
                route = "/" + "/".join(parts) if parts else "/"
                if "[" not in route:
                    homes[role] = route
        data["role_homes"] = homes if len(role_keys) >= 2 else {}
