...
class ArchitectWriteMixin:
    ...
    def _uncovered(self, plan: dict) -> list:
        ...
        Only meaningful when the brief had numbered requirements to begin
        with — `_known_fr` is what `make_plan` read out of it — so a plain
        prompt with no specification returns nothing and no gate fires.
        Sorted numerically, so FR-002 comes before FR-010 in the log.
        """
    ...
    def _routes_promised(self, markdown: str) -> list:
        ...
        Only two shapes count, because only two are structural:
          • a row of the ## Routes table — the path is its first cell;
          • a step of a Page Flow chain — `/ → /rooms → /rooms/[id]`.
        Everything else in a plan is prose, and prose full of slashes.
        """
        out, seen = [], set()
    ...
    def _route_of(path: str) -> str:
        """`app/rooms/[id]/page.jsx` → `/rooms/[id]`; `app/page.jsx` → `/`.

        Route groups — `app/(shop)/cart/page.jsx` — are a Next.js folder
        convention that does not appear in the URL, so they are dropped.
        """
    ...
    def _unbuilt_routes(self, plan: dict, markdown: str) -> list:
        ...
        The prompt has always said "every route you list in ## Routes must
        have a matching page.js" and "never name a route here that is not in
        that table" — and prose is not a check. Nothing counted, so a plan
        could name `/rooms/[id]` in its Routes table and its Page Flow, put a
        link to it on the catalogue, and simply never list the file. The page
        is then a 404 in a finished app, which is what "every app has half a
        page missing" looks like from the outside.

        So the two lists are compared: what the markdown promises against
        what the tasks will actually write. A promised route matches a
        planned one segment for segment, with `[id]`-style segments matching
        anything — so `/rooms/507f…` in a Page Flow example still matches the
        planned `/rooms/[id]`.
        """
        planned = set()
        for phase in (plan or {}).get("phases") or []:
