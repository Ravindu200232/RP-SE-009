"""Task synchronization, build loop and generated-file orchestration."""
...
class ArchitectBuildMixin:
    ...
    def _unplanned_imports(self, rel: str) -> list:
        ...
        The per-turn gate tolerates an import whose target has not been written
        yet, because the plan may write it three turns later. It must not
        tolerate an import of something the plan never mentions: nothing will
        ever create it, and the failure surfaces at `next build` instead --
        which on the measured bakery build cost a whole extra round for
        `@/components/AddToBasketButton`, a component the plan never named.

        Returns [(spec, message)]. Empty when every import is either resolvable
    ...
    def _acceptance_lines(self, f: dict) -> list:
        ...
        The analyzer checks contracts and data edges as literal strings after
        the build. A builder that was never told the literal writes the same
        behaviour through a helper, fails the grep, and buys a repair round.
        Saying the exact string here is the cheapest round ever removed.
        """
    ...
    def _import_whitelist_block(self) -> str:
        ...
        The per-turn gate catches a phantom import after the file is written —
        a whole turn late. Listing what is importable up front is what stops
        `@/lib/auth` being invented in the first place.
        """
        allowed = set()
    ...
    def _file_list_block(self, files: list) -> str:
        """Each file with its whole brief: purpose, then every section, then
        every action. This is what the turn that writes the file reads, so it
        is where a thin page becomes a full one — a builder shown four named
        sections writes four; shown "dashboard, reads tasks" it writes one."""
        out = []
        for f in files:
    ...
    def build_app(self) -> int:
        ...
        if self.plan.get("demo_accounts"):
            ...
            extras += (f"\n\nDemo accounts (seed these EXACT values through Better Auth; "
                       f"then assign each exact planned role. Do NOT display them "
                       f"anywhere in the UI): {accounts}")
            homes = self.plan.get("role_homes") or {}
            if isinstance(homes, dict) and homes:
                mapping = ", ".join(f"{role} -> {route}" for role, route in homes.items())
                extras += ("\nRole landing routes (authoritative): " + mapping +
                           ". After signIn.email, read the returned/refreshed session "
                           "role and route that exact role here; never send all roles "
                           "to the same page.")
