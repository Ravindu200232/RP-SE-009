...
The plan and the SRS are text. Nobody can tell from "Storefront — browse the
catalogue with filters" whether they will like what they get, so the first
picture a customer ever saw arrived after a full build — forty files and
several minutes in, which is the most expensive place in the whole product to
discover "no, not like that".

This is that picture, five times, before the build starts. The customer picks
one and the choice becomes `design.md`, which the builder is bound by — so what
was approved is what gets built, rather than a mood board the build is free to
ignore.

How the five are made — three decisions, each of them a reversal of what was
here before, and each made because the earlier version was tried:

1. THE DESIGNER GETS PLAN.MD. The demo used to be drawn from a
   one-paragraph intent and three picked pages. Now the approved plan as the
   customer read it — what it is, who uses it, the screens, the records, the
   user journeys, the look and feel — is what the designer reads, so the demo
   is a picture of the application that will be built, not of a summary of
   it. NOT the builder's handoff: that document is written to make a Next.js
   application, and a model handed it draws Next.js — routes, components, a
   build — where a plain HTML picture was wanted.

2. ONE CALL PER DESIGN, ONE FILE, THREE REPRESENTATIVE SCREENS. It used to
   spend the whole response on one landing page, which proved a palette but not
   whether a dashboard, form and detail page belonged to the same product. Each
   design is still one response and one HTML document, but when the plan has
   enough screens it shows three substantial representatives: the primary
   surface, the core-workflow surface and a management/detail/completion surface.
   Five calls, five coherent application previews, in parallel.

3. NO NAMED DIRECTIONS. "Editorial / Utility / Soft / …" were written here in
   advance and every run drew the same five. They are gone. Each call gets a
   RANDOM starting direction instead — a palette family, a type voice, a
   shape, a density and one signature move, drawn fresh from wide pools every
   run, no two designs sharing a palette or a type voice — and is told the
   rest is its own decision. "Draw again" therefore means five NEW designs,
   not the same five redrawn. The random seed exists because five identical
   prompts to one model return five variations on one idea (the reason the
   named directions were written in the first place); the seed keeps the
   variety without keeping the presets.

The previews are self-contained HTML, not JSX. AgentForge runs with no
network, so a preview cannot pull React, Babel and Tailwind off a CDN, and
there is no build step available before the build. Inline CSS in an iframe is
the only thing that renders honestly offline.
"""
...
# Random directions Wide pools, drawn from at random on every run.
_PALETTES = [
...
_MOVES = [
    ...
    "big rounded avatars and product images as the primary visual",
...
def random_directions(n: int = COUNT, rng: random.Random | None = None) -> list[dict]:
    ...
    A `rng` can be passed for a repeatable draw (tests); by default every run
    draws afresh, which is what makes "Draw again" five new designs.
    """
...
- Where a photograph belongs, write only a marker such as:
    <img src="{{photo: boutique hotel room}}" alt="…">
...
- Use real-looking domain content — products, prices, dates, names, statuses —
...
def brief_from_plan(plan: dict | None, doc: dict | None, idea: str = "") -> str:
    ...
    The handoff is the SRS's — it exists once an SRS has been approved. A
    bare prompt has none, and a partly-done SRS has a plan and a document but
    no handoff; this writes the nearest thing from whatever is there, in the
    handoff's own shape (what it is, who uses it, the pages), so the one
    prompt below reads the same either way.
    """
    plan = plan or {}
    doc = doc or {}
    lines = []
...
(function(){
  ...
  function pick(slug){
    ...
    // "admin" for "admin-books", "orders" for "my-orders": nearest name wins
...
def finish(raw: str) -> str:
    ...
    A single long response can end early — the output budget runs out mid
    table — and the alternative to repairing it is a blank cell in the picker
    for a design that is nine-tenths there. So: fences stripped, a cut-off
    document closed, the model's scripts removed and the one page-switch
    script supplied — always ours, never the model's. Never raises.
    """
    t = (raw or "").strip()
    if t.startswith("```"):
        t = t.split("\n", 1)[1] if "\n" in t else ""
...
def pages_of(html: str) -> list[str]:
    ...
    The nav's own labels where they exist (`data-goto="x"` inside a <nav> →
    the link text), else the slug made readable. Only the <nav> counts: a
    "Proceed to Checkout" button and a book title on a card also carry
    data-goto, and on the first live run both became "page names" in the
    picker's header.
    """
    ...
    for nav in navs:
        for tag, slug, inner in re.findall(
            ...
            text = re.sub(r"\s*\(\s*\d+\s*\)\s*$", "", text)  # "Cart (2)" → "Cart"
...
def design_prompt(html: str, page_name: str = "") -> str:
    ...
    `page_name` is accepted and deliberately unused. It used to be handed over
    as "It contains these pages: …", which is a list of the demo's structure
    given to a reader whose whole job is the look — and the document it writes
    is put in front of the builder as rules to follow. Naming the pages there
    invited a second plan, competing with the one that had already decided
    them.
    """
    del page_name
