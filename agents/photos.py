...
The model that writes a demo page never sees a URL. It marks where a photograph
belongs — `{{photo: a rack of folded shirts}}` — and this fills them in
afterwards. Three reasons it works that way:

- A model asked to invent image URLs invents ones that 404, and a demo full of
  broken images reads as a broken design.
- The fetch happens once, here, and the bytes are inlined as a `data:` URI. The
  finished HTML then renders with no network at all, which is what the rest of
  AgentForge assumes and what makes the preview honest.
- One search phrase per slot is cacheable and cheap: the same phrase in five
  themes is one download, not five.

Unsplash first, Pexels second, and a drawn placeholder if neither is
configured — a missing API key must never be the reason a demo cannot be
shown, it just means the demo shows blocks instead of photographs.
"""
...
def _openverse(phrase: str, _key: str = "") -> str:
    ...
    This is the tier that makes photographs work on a fresh install: Unsplash
    and Pexels are better pictures but need a key nobody has on day one, and
    until it was added the demos went out with grey blocks that said "minimal
    luxury hotel exterior" where the hotel should be. Openverse's index is
    Flickr, Wikimedia and museums under CC licences — sometimes amateur, always
    real, always allowed. `aspect_ratio=wide` because a demo's photograph
    slots are almost all landscape.
    """
...
def _placeholder(phrase: str) -> str:
    ...
    Still a real `src`, so the markup the customer approves is the markup the
    build inherits — only the picture inside it is missing.
    """
...
def fill(html: str, settings: dict | None = None,
    ...
    """Replace every `{{photo: …}}` with an inlined image. Never raises.

    Returns the html and how many real photographs went in, so the caller can
    say "6 photographs" or "no key, using placeholders" rather than leaving
    somebody to wonder why the demo is grey.
    """
