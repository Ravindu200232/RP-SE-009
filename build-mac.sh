#!/bin/bash
# =============================================================================
# build-mac.sh — Build AgentForge.dmg for macOS
...
echo -e "${BOLD}"
echo "  ╔════════════════════════════════════╗"
echo "  ║    AgentForge macOS DMG Builder        ║"
echo "  ╚════════════════════════════════════╝"
echo -e "${RESET}"
...
if command -v gtimeout >/dev/null 2>&1; then
    ...
    dist-backend/locode-backend-v4 >/tmp/_agentforge_test.txt 2>&1 &
    BG_PID=$!
    sleep 3
    kill $BG_PID 2>/dev/null || true
    BACKEND_OUT=$(cat /tmp/_agentforge_test.txt 2>/dev/null || true)
fi
...
if [ -d "$MOUNT_DIR/AgentForge.app" ]; then
    RES="$MOUNT_DIR/AgentForge.app/Contents/Resources"
...
echo "  📏 Size: $DMG_SIZE"
echo ""
echo "  To install: open '$DMG' and drag AgentForge to Applications"
echo ""