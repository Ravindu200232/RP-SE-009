...
export default function SandboxStudio({ onNavigate, onToast }) {
  ...
  const lastErrorSigRef = useRef(null);
  const lastRepairFinishedAtRef = useRef(0);
  ...
  const startBuildFromSrs = async (rough, userText) => {
    ...
    lastRepairFinishedAtRef.current = 0;
  ...
  const silentAutoRepair = async ({ message, stack }) => {
    ...
    try {
      ...
      pollSandbox(id, ({ ready, failed }) => {
        repairInFlightRef.current = false;
        lastRepairFinishedAtRef.current = Date.now();
        if (ready) {
          setPhase("ready");
        } else if (failed) {
          setPhase("ready");
  ...
  // Reduce a runtime error to a short, comparable signature so we can detect
  // when the same bug is being re-emitted by the model.
  const errorSignature = (msg) => {
    const s = String(msg || "").trim().toLowerCase();
    // Cap length so trailing details (line numbers, ids) don't make every
    // re-occurrence look unique.
  ...
  errorHandlerRef.current = ({ message, stack }) => {
    ...
    // Cooldown: ignore errors that fire immediately after a repair just
    // finished — they are usually the previous (cached) iframe load
    // emitting one final death rattle, not a fresh failure.
    const now = Date.now();
    if (now - lastRepairFinishedAtRef.current < REPAIR_COOLDOWN_MS) return;

    const sig = errorSignature(message);

    // If we already attempted at least one repair and the SAME error came
    // back, the model isn't fixing it. Stop looping and surface the gentle
    // amber overlay so the user can refine in chat.
    if (repairAttemptRef.current > 0 && sig === lastErrorSigRef.current) {
      setRuntimeError({ message, stack, ts: now });
      return;
    }
