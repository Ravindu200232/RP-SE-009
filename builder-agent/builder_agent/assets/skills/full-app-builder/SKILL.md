...
1. **Generate the application.** Convert the requirements into one coherent architecture and implement complete vertical flows, including validation, authorization, persistence, errors, loading/empty states, navigation, and responsive UI. Read the `frontend-design` and `page-composition` skills at the moment you start the screens, not in the opening sweep, and apply them as you write: composition, spacing rhythm and hierarchy are decided in the markup, and a styling pass afterwards can only tidy what the structure already got wrong. `page-composition` carries the section counts a page of each kind needs and the order they argue in; reading it thirty files before the first screen is written is the same as not reading it, and a one-paragraph home page is what comes back. Write the screens as their own stretch of work rather than trailing them after the API routes, and finish one screen before starting the next — two screens in one reply get one reply's worth of attention between them. Do not stop at a scaffold or a collection of disconnected pages. Finish every file the accepted scope needs before running any verification: no test file, no build, no browser until the application itself is complete.
