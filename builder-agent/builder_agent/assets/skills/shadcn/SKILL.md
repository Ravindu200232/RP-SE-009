...
The selected Shadcn scaffold and provider blocks are installed before the builder starts. Read `.agents/skills/design-system/blocks.json` and use its installed paths. Do not reinstall selected blocks, replace them with smaller lookalikes, or switch registries during the build. The approved plan already settles product scope; implement it without pausing for another registry-choice question.
