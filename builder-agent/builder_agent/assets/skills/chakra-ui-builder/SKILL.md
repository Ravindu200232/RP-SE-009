...
The Chakra v3 provider and dependencies are already installed in the selected Next.js root or MERN client. The approved plan and design contract settle the product, palette, and route scope, so proceed from them without another design question. Full Chakra page blocks require a Chakra Pro API key; when no selected provider source exists, compose the planned interface from official Chakra primitives and snippets without inventing catalogue entries.
