...
5. Prefer readFiles to inspect multiple related files (such as prototype pages, routes, or configs) in a single turn. Batch genuinely independent operations in one response; the engine executes them in order and returns every result. Keep dependent actions ordered and read their results before work that relies on them.
...
Put requirement ids in runTests.covers and browserRunJourney(s).covers. Mark project-changing shell commands with changesProject:true; services and read-only commands are false.
...
For E2E, cover every sealed critical journey at its real public boundary with browserRunJourneys in the engine's isolated browser when there is more than one suite; use one browserRunJourney only for a single suite. Give each suite a stable startUrl. Accessible names resolve exact, then normalised, then one unique containing match - do not guess long concatenated names. The batch runner executes every suite and records each one separately, even when one fails, so do not call the suites one by one. After a failure, read the bounded URL, diagnostics and page text the journey already returned; do not take another snapshot of an unchanged page. Route repair by owner: E2E_SELECTOR_* means fix the journey locator, E2E_UI_TARGET_MISSING means fix the product UI, route or state, and an assertion, console, network or 5xx failure means fix production behaviour. A suite that failed twice with no repair in between will fail a third time; repair the owner instead.
...
Use isolated test data, accounts and ports. Never touch production data or real payments. Isolated means a different database name, not a different variable name: a suite whose TEST_URI falls back to the application's own connection string is running against the app, and the first deleteMany empties what the app is serving. Setup, build and lint alone, screenshots, skipped suites and masked failures are not test passes.
