...
# ---------------------------------------------------------------------------
# The agent-facing side of the transport
# ---------------------------------------------------------------------------
def estimate_tokens(text) -> int:
    """Cheap, deliberately pessimistic token estimate.

    A real tokenizer per provider would be more accurate and would also make
    every context decision depend on a model download. The loop only needs to
    know when it is approaching a boundary, and over-estimating by a few
    percent costs one early checkpoint rather than a truncated request.
    """
    if isinstance(text, str):
        return len(text) // 3 + 1
    try:
        return len(json.dumps(text, ensure_ascii=False)) // 3 + 1
    except (TypeError, ValueError):
        return len(str(text)) // 3 + 1


class ToolCall:
    """One native tool call the model asked for."""

    __slots__ = ("call_id", "tool", "args")

    def __init__(self, call_id: str, tool: str, args: dict):
...
class Reply:
    """What one model turn produced: text, calls, and how much it cost."""

    __slots__ = ("content", "calls", "usage", "truncated", "thinking")

    def __init__(self, content="", calls=None, usage=None, truncated=False, thinking=""):
...
class Router:
    """The model, as the loop sees it.

    Owns the active model, streams tokens to the event bus so a studio can show
    a build thinking, and tracks cumulative usage. Native tool calling only:
    tool intent never arrives as text to be parsed out of a chat reply, because
    a parser that executes model prose is a prompt-injection surface with a
    convenience feature attached.
    """

    def __init__(self, client, model: str, config=None, events=None):
        self.client = client
        self.model = model
        self.config = config
        self.events = events
    ...
    def context_window(self) -> int:
        try:
            return max_context(self.model)
        except Exception:  # noqa: BLE001 - a probe failure is not a run failure
            return 0

    def supports_vision(self) -> bool:
        try:
            return self.client.supports_vision(self.model)
        except Exception:  # noqa: BLE001
            return False

    def ask(self, messages, tools=None, stream: bool = True, think=None,
            temperature: float = None, timeout: int = 1800) -> Reply:
        ...
        for chunk in self.client.chat_stream(self.model, messages, tools=tools or None,
            ...
            if token:
                ...
                if self.events:
                    self.events.emit("token", token=token)
        ...
        return self._reply(message, final)

    def _reply(self, message: dict, envelope: dict) -> Reply:
        calls = []
        for index, raw in enumerate(message.get("tool_calls") or []):
            ...
            if isinstance(arguments, str):
                try:
                    ...
                    arguments = {}
            calls.append(ToolCall(raw.get("id") or f"call_{index}_{int(time.time()*1000)}",
                                  str(function.get("name") or ""), arguments))
        usage = {"prompt": int(envelope.get("prompt_eval_count") or 0),
                 "completion": int(envelope.get("eval_count") or 0)}
        self.usage["prompt"] += usage["prompt"]
        self.usage["completion"] += usage["completion"]
        self.usage["requests"] += 1
        # Ollama reports "length" when the model hit its output ceiling mid-answer.
        truncated = str(envelope.get("done_reason") or "").lower() == "length"
        return Reply(content=message.get("content") or "", calls=calls, usage=usage,
                     truncated=truncated, thinking=message.get("thinking") or "")
