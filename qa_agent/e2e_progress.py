...
Progress means the browser moved through the business journey, not merely that
the error text changed.  Keeping this policy pure makes it cheap to test and
prevents orchestration code from silently redefining convergence.
"""
...
def measure_progress(before, after, seen_signatures: set, *,
    ...
    A different message or lower-severity symptom at the same checkpoint is not
    progress.  This prevents a loop from resetting its no-progress budget while
    repeatedly rewriting code around the same failing step.
    """
...
def stop_after_no_progress(round_no: int, progressed: bool, minimum_rounds: int = MIN_REPAIR_ROUNDS,
    ...
    A short evidence budget is useful while the debugger still has distinct,
    evidence-backed hypotheses.  It must not force extra model turns after the
    notebook proves the only available hypothesis was already rejected by the
    browser.  That is repetition, not agentic investigation.
    """
...
def extend_round_budget(round_no: int, budget: int, hard_cap: int, bonus: int,
    ...
    The default two rounds keep stuck journeys cheap. If the last budgeted round
    actually moves the business journey to a later step, stopping there throws
    away demonstrated progress.  Extend in small chunks, up to a
    separate hard cap; same-step symptom churn never earns extra work.
    """
    try:
