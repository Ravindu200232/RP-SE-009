...
def _e2e_final_clean_room(agent, arch, proj_dir: Path, qa, analyzer,
    ...
    if failures:
        ...
        for flow, journey in zip(flows, journeys):
            title = str(flow.get("title") or "")
            scenario = agent.accepted_scenario(journey)
            if results.get(title):
                flow["clean_room"] = "fail"
                flow["failed"] = len(results[title])
                if scenario is not None:
                    flow.update(stage_result(scenario, results[title], {}))
            else:
                flow["clean_room"] = "pass"
                if scenario is not None:
                    flow.update(stage_result(scenario, [], {"completed": True}))
