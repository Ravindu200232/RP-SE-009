...
from qa_agent.unit.author_common import *
...
class UnitAuthorWriteMixin:
    ...
    def _preamble_note(target_src: str) -> str:
        ...
        """
        from qa_agent.core.session_common import mock_line, required_mocks
    ...
    def _phase_goal(self, phase):
        ...
        `phase` is 1-based, and `phase=0` means "not part of a phase" — the
        backfill pass and the feature path both use it. Indexing `phase - 1`
        with 0 reads `phases[-1]`, so those batches were labelled with the
        LAST phase's goal: a test for the public catalogue told it belonged to
        "Polish & UX". A wrong heading is worse than none.
        """
        if not phase:
            return ""
        try:
    ...
    def _brief_for(self, path: str) -> str:
        ...
        The same purpose / sections / actions the builder was shown on the
        turn it wrote the file. Without it a test can only assert what the
        code already does, and a missing feature produces a passing test
        written from the code that is missing it.
        """
        try:
    ...
    def write_for(self, targets, phase=0, max_reads=MAX_READS):
        ...
        for t in targets:
            ...
            blocks.append(f"--- {t.path} ---\n{body}\n\n"
                          + (f"What the plan asked this file for — assert "
                             f"these, and let a missing one fail:\n{brief}\n\n"
                             if brief else "")
    ...
    def _target_unfinished(self, t) -> str:
        ...
        The test under repair imports its component; that component imports
        others. If any of those is still in the plan and not yet written, the
        test cannot pass however it is rewritten — the fix belongs to the
        build turn that writes the missing file, not to a repair round here.
        """
    ...
    def _verify(self, convo, assigned, written, rejected, phase):
        ...
        from qa_agent.unit.harness_common import npm_busy
...
from qa_agent.unit.author_repair import UnitAuthorRepairMixin
from qa_agent.unit.author_guards import UnitAuthorGuardMixin


class UnitTestAuthor(UnitAuthorWriteMixin, UnitAuthorRepairMixin, UnitAuthorGuardMixin):
    """Concrete unit-test author/repair/quality-guard agent."""
    pass
