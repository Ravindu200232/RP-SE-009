...
* **Never `networkidle`.** `agents/build/tester.py:288` already documents why: the
...
from qa_agent.e2e.e2e_semantics import js_runtime_value
...
ASSERTIONS = {"EXPECT_TEXT", "EXPECT_URL", "EXPECT_VALUE", "EXPECT_MUTATION",
              "EXPECT_NO_ERROR"}
...
def field_css(key: str) -> str:
    ...
    Known fields get hand-tuned browser contracts. Unknown-but-safe names are
    still useful when they came from runtime/source evidence: bind by
    name/id/aria/placeholder rather than
    dropping the E2E step and shrinking the journey.
    """
...
# name= is optional; a bare role=combobox means "any combobox", not page text.
...
_PLACEHOLDER_RE = re.compile(r"\{\{\s*(email|password|role)\s*\}\}", re.I)
_SECRET_QUERY_RE = re.compile(
    r"[?&](?:password|passwd|passcode|token|secret|authorization|session)\s*=",
    re.I)
...
class Scenario:
    title: str = "user flow"
    role: str = ""
    # A public actor can cross an authentication boundary during one journey.
    # Keep that target identity separate from the actor that starts the flow.
    auth_role: str = ""
    ...
    def is_runnable(self) -> str:
        ...
        unresolved = sorted({
            ...
            for m in _PLACEHOLDER_RE.finditer(str(getattr(step, "value", "") or ""))
        })
        if unresolved:
            return ("unresolved authentication placeholder(s): "
                    + ", ".join("{{" + name + "}}" for name in unresolved)
                    + "; select an exact demo identity before browser execution")
        return ""
        ...
        if kind == "field":
            ...
            if not field_css(key):
                return None, (f"unsafe field={pat!r}; use an observed simple "
                              "field name from the observed DOM/source")
            pat = key
        if kind == "href":
            pat = pat.strip()
            if not pat.startswith("/") or pat.startswith("//") or \
                    not re.fullmatch(r"/[A-Za-z0-9_./?=&%#:+~-]*", pat):
                return None, "href= must be an observed same-app absolute path"
...
def parse_url_selector(raw: str):
    ...
    The scenario model is told to write JavaScript-style regex literals such
    as ``/records\\/.*/``. In practice it occasionally writes the visually
    obvious shorthand ``/records/.*`` or even ``text=/records/.*``. The generic
    selector parser quite reasonably treats that as a literal string, which
    makes a concrete dynamic URL fail even though the authored intent
    is unambiguous.

    URL assertions are their own grammar, so normalize only here. Plain routes
    stay literal; route-shaped wildcard/template forms become anchored-enough
    regexes. We do not reinterpret arbitrary page text as a URL pattern.
    """
...
def parse_scenario(text: str, *, account: dict = None) -> Scenario:
    ...
    """
    account = account or {}
    sc = Scenario(auth_role=str(account.get("role") or "").strip().lower())
    last_click = ""
    ...
    for i, raw in enumerate((text or "").splitlines(), 1):
        ...
        if not line or line.startswith(("#", "//")):
            continue
        if "::" not in line:
            if line.upper() in VERBS and VERBS[line.upper()] == 0:
                sc.steps.append(Step(verb=line.upper(), line=i))
        ...
        if verb == "GOTO":
            ...
            if not path.startswith("/"):
                sc.dropped.append((i, line[:80], "GOTO needs an observed same-app absolute path"))
                continue
            ...
            if "[" in path or "]" in path:
                ...
                sc.dropped.append((i, line[:80],
                                   ...
                                   "by clicking a link"))
                continue
            if _SECRET_QUERY_RE.search(path):
                sc.dropped.append((i, line[:80],
                                   "credentials/tokens must never be placed in a GOTO URL"))
                continue
...
def _fill_placeholders(value, account):
    ...
    def sub(m):
        return str(acc.get(m.group(1).lower(), "")) or m.group(0)
    return _PLACEHOLDER_RE.sub(sub, value)[:200]


def is_auth_submit_step(sc: Scenario, index: int) -> bool:
    """Whether one CLICK submits credentials rather than opening a login link."""
    if not getattr(sc, "auth_role", "") or index < 0 or index >= len(sc.steps):
        return False
    step = sc.steps[index]
    if step.verb != "CLICK":
        return False
    sel = getattr(step, "selector", None)
    label = " ".join((str(getattr(sel, "pattern", "") or ""),
                      str(getattr(sel, "role", "") or ""))).lower()
    if not re.search(r"sign.?in|log.?in|login|authenticate", label):
        return False
    fields = set()
    for prior in reversed(sc.steps[:index]):
        if prior.verb == "GOTO":
            break
        if prior.verb != "FILL" or not prior.selector:
            continue
        text = " ".join((str(prior.selector.pattern or ""),
                         str(prior.selector.role or ""))).lower()
        if "email" in text:
            fields.add("email")
...
def to_playwright_js(sc: Scenario, *, base_url="http://localhost:5173") -> str:
    ...
    body = [
        ...
        "    if (r.status() < 300 && ['POST','PUT','PATCH','DELETE'].includes(method)",
        "        && !new URL(r.url()).pathname.startsWith('/api/auth/'))",
        "      mutations.push({ method, url: r.url(), status: r.status() })",
        "  })",
        "",
    ]
    for index, st in enumerate(sc.steps):
        body.append("  " + _js_step(st))
        if is_auth_submit_step(sc, index):
            body.extend([
...
def _js_step(st: Step) -> str:
    if st.verb == "GOTO":
        ...
        return (f"await page.goto(BASE + {js_string(st.value)}, "
                f"{{ waitUntil: 'load' }})")
    if st.verb == "FILL":
        return ("mutationMark = mutations.length\n  "
                f"await page.{st.selector.js_locator()}.first().fill("
                f"{js_runtime_value(st.value, js_string)})")
    if st.verb == "SELECT":
        loc = f"page.{st.selector.js_locator()}.first()"
        value = js_runtime_value(st.value, js_string)
        return ("mutationMark = mutations.length\n  "
                f"await {loc}.selectOption({{ label: {value} }})"
                f".catch(async () => {loc}.selectOption({value}))")
    if st.verb == "CLICK":
        return ("mutationMark = mutations.length\n  "
                f"await page.{st.selector.js_locator(clickable=True)}"
                f".first().click()")
    if st.verb == "WAIT_FOR":
    ...
    if st.verb == "EXPECT_VALUE":
        return (f"await expect(page.{st.selector.js_locator()}.first())"
                f".toHaveValue({js_runtime_value(st.value, js_string)}, {{ timeout: 15000 }})")
    if st.verb == "EXPECT_MUTATION":
        return ("await expect.poll(() => mutations.length, { timeout: 15000 })"
                ".toBeGreaterThan(mutationMark)")
...
Selectors — one of these semantic forms, and nothing else:
    ...
    field=email
    field=password
    field=observedFieldName
    placeholder=/search/i
    text=/welcome back/i
    testid=observed-control-id
    href=/observed-static-path

For login/authentication fields prefer `field=email` and `field=password`.
Those target standard HTML input type/name/autocomplete and survive placeholder
wording such as "name@example.com". For product fields, use the simple name or
id actually observed in DOM/source. A simple observed field name binds by
...
For date inputs whose requirement does not demand a specific historical date,
use `{{date+7}}`, `{{date+8}}`, etc. These values are resolved when the test is
run, so an exported test does not become invalid merely because time passed.

After a create/update/delete business action, use EXPECT_MUTATION to prove that
the browser observed a successful non-auth POST/PUT/PATCH/DELETE. Pair it with a
URL, entered value, or record-specific assertion when the UI exposes one. Do not
assert that an entire list is empty unless the approved requirement guarantees
there can be no other records.
