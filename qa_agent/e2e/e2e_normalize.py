...
from qa_agent.e2e.flows import Selector, parse_selector
...
def normalize_scenario_selectors(sc) -> list[str]:
    ...
    for step in getattr(sc, "steps", []) or []:
        ...
        # role= now parses natively even without name=, so the wrapped shape
        # can arrive here already unwrapped — as EXPECT_TEXT on a role, which
        # would grep page text with the role's pattern. It means presence.
