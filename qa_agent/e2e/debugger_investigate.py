...
from qa_agent.e2e.debugger_common import *
...
class DebuggerInvestigateMixin:
    ...
    def investigate(self, failures, *, journey=None, scenario=None,
        ...
        if deterministic:
            ...
            learned_rejection = self.notebook.learned_rejection(deterministic)
            if learned_rejection:
                # Historical failure changes the repair strategy, never the
                # evidence-backed verdict (for example, a legitimate business
                # precondition must not become APP_FIX merely because one old
                # fixture rewrite failed).
                deterministic["learned_stalled"] = True
                deterministic["next"] = (
                    str(deterministic.get("next") or "").rstrip(". ")
                    + ". Prior runs did not progress with this same diagnosis; "
                      "inspect the captured state and use a materially different "
                      "setup/repair path instead of repeating the previous attempt."
                )
            if not dh or dh not in self.notebook.rejected_hypotheses or learned_rejection:
        ...
        pure_selector = bool(failures) and all(
            ...
            for f in failures)
        if pure_selector and scenario is not None:
            try:
        ...
        for _turn in range(MAX_TOOL_TURNS):
            ...
            if ((hyp and hyp in self.notebook.rejected_hypotheses)
                    or self.notebook.learned_rejection(decision)):
                transcript.append(
                    "## Controller feedback\nThat exact diagnosis already failed to move the browser. "
    ...
    def _add_producer_page(self, decision: dict) -> dict:
        ...
        A navigation contract always lives in two files: the page that writes
        the link and the page that reads what the link carried. The failure
        only ever surfaces on the second one, so resolving a target from the
        failing URL alone puts the fixer inside the consumer and leaves the
        defect untouched -- and every round after that re-edits the same
        innocent file.

        In a measured build, a dynamic producer linked to its consumer without
        the query value that consumer read. The value was null, downstream
        state never loaded, and the journey's evidence could not render. The
        loop rewrote the consumer three times and never opened the producer.

        Network evidence cannot show this: every request in that run returned
        200. Only the two sources side by side do.

        `route_before` is already recorded on every failure -- this only
        spends it.
        """
        try:
            ev = dict(getattr(self.agent, "_last_run_evidence", {}) or {})
    ...
    def propose_test_patch(self, failure, scenario=None) -> str:
        ...
        Prefer deterministic browser evidence. The model is only the fallback
        for cases where the DOM inventory cannot name a unique equivalent
        control.
        """
        try:
...
TEST_PATCH :: CLICK :: role=button name=/observed action/i
...
"""
        ...
        line = next((ln.strip() for ln in str(raw or "").splitlines()
                     if re.match(r"^(GOTO|FILL|SELECT|CLICK|EXPECT_TEXT|EXPECT_URL|EXPECT_VALUE|EXPECT_MUTATION|WAIT_FOR|EXPECT_NO_ERROR)(?:\s*::|\s*$)", ln.strip(), re.I)), "")
        return line[:1200]
    ...
    def _runtime_locations(self, text: str) -> list[dict]:
        ...
        Only paths that exist in Architect.files are accepted.  This prevents a
        bundled `_next/static/chunks/...` URL or a node_modules frame from
        becoming a bogus repair target.  Windows paths and webpack-internal
...
from qa_agent.e2e.debugger_reasoning import DebuggerReasoningMixin


class AgenticE2EDebugger(DebuggerInvestigateMixin, DebuggerReasoningMixin):
    """Concrete evidence-first E2E debugger."""
    pass
