...
Every other gate in the pipeline asks whether the app *works*. This one asks
whether it is safe to put in front of people, and it is the only gate that can
pass an app which compiles, renders, and answers every route correctly. A
`POST /api/reservations/bulk-update` that never asks who is calling is a working
feature and an open door; nothing in the build, the unit tests, the browser pass
or the route probe has any opinion about it. Measured on a shipped project.

Two halves, and the split is deliberate.

**`npm audit`** answers the question nobody can answer by reading code: which of
the four hundred packages under `node_modules` has a published advisory today.
It costs one command and needs the network.

**The code checks** are written the way `AnalyzerAgent`'s are — deterministic,
specific, and only where a false positive is impossible. That constraint is what
makes them worth wiring to the repair loop: a check that cries wolf teaches the
pipeline to rewrite correct code, which is the one outcome worse than shipping
the flaw. So `GET /api/products` with no session check is NOT reported — a
public catalogue read is meant to be public — while `POST` to the same path is.

Findings come back as `analyzer.Finding`, because the repair path already
consumes that shape and there is no reason for a second one.
"""
import json
import os
import logging
...
class SecurityAgent:
    ...
    def _sources(self):
        ...
        `os.walk` with the skip list applied to `dirnames`, not
        `glob("**/*.js*")` with the skip list applied to the results. The glob
        walks `node_modules` in full — tens of thousands of files — and then
        throws them away, which took this pass from instant to minutes per
        project. Pruning the directory is the difference.
        """
    ...
    def unguarded_routes(self) -> list:
        ...
        for rel, body in self._sources():
            ...
            if not (set(writes) & set(ALWAYS_GUARD)
                ...
                continue
            out.append(Finding(
                severity="blocker", code="UNGUARDED_ROUTE", path=rel,
                message=(f"{', '.join(writes)} changes records that already "
                         f"exist and never checks who is calling — anyone who "
                         f"knows the URL can use it"),
                fix=("Read the session at the top of each write handler and "
    ...
    def unguarded_pages(self) -> list:
        ...
        Next has no inherited guards: `app/reception/page.jsx` redirecting the
        wrong role says nothing about `app/reception/book/page.jsx`. A section
        page marked `'use client'` cannot read the session at all, so whatever
        it shows, it shows to anyone who types the URL.

        Measured on a four-role clinic: `/reception` correctly bounced a nurse
        and `/reception/book` — the page that actually creates appointments —
        answered them 200. Every other gate passed the app, and the API check
        below never looks at pages.

        Only sections that ARE gated somewhere are considered, so a wholly
        public site is never reported.
        """
        pages, guarded = {}, set()
    ...
    def audit(self) -> dict:
        """`npm audit`, as `{severity: count}`. Empty when it could not run."""
        if not self.cmd:
            return {}
    ...
    def run(self, *, audit: bool = True) -> tuple:
        ...
        Dependency audit is intentionally optional.  The main security stage
        runs `npm audit` once; if source findings are repaired, its confirmation
        pass rescans source only instead of downloading/recomputing the same
