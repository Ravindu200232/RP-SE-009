...
class E2EStepsMixin:
    ...
    def _concept_words(text: str) -> set:
        ...
        out = set()
        for w in words:
            if w in {"booking", "bookings", "reservation", "reservations",
                     "reserve", "reserved"}:
                out.add("booking")
            elif w in {"room", "rooms"}:
                out.add("room")
            elif w in {"payment", "payments", "paid", "pay"}:
                out.add("booking")
                out.add("payment")
            elif w in {"price", "pricing", "rate", "rates", "cost"}:
                out.add("pricing")
                out.add("room")
            elif len(w) > 3 and w not in {"management", "manage", "page",
                                          "view", "all", "the"}:
                out.add(w)
        return out
    ...
    def warm(self, sc):
        ...
        for path in sorted(targets):
            if path in self._warmed_routes:
                continue
            try:
                requests.get(self.base_url + path, timeout=15)
