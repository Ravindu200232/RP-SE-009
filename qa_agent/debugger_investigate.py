...
class DebuggerInvestigateMixin:
    ...
    def _add_producer_page(self, decision: dict) -> dict:
        ...
        Measured on the hotel build: app/rooms/[id]/page.jsx linked to
        `href="/checkout"` with no query string while app/checkout/page.jsx
        read `searchParams.get('roomId')`. roomId was null, so the price was
        never fetched, the total stayed null, and the "(Discounted 10%)" line
        the journey asserted on could not render. The loop rewrote
        app/checkout/page.jsx three times across four rounds and never opened
        the page that produced the link.
...
TEST_PATCH :: CLICK :: role=button name=/add to cart/i
