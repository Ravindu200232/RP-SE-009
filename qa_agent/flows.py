...
FIELD_CSS = {
    ...
    "end": ('input[name*="end" i], input[name*="checkout" i], '
            'input[name*="check-out" i], input[name*="to" i]'),
    ...
    "room": ('select[name*="room" i], input[name*="room" i], '
             '[role="combobox"][aria-label*="room" i]'),
...
def field_css(key: str) -> str:
    ...
    still useful when they came from runtime/source evidence (guestCount,
    nightlyRate, specialRequest): bind by name/id/aria/placeholder rather than
    ...
    if m:
        ...
        if kind == "href":
            ...
            if not pat.startswith("/") or pat.startswith("//") or \
                ...
                return None, "href= must be a same-app path such as /bookings/new"
...
def parse_url_selector(raw: str):
    ...
    as ``/rooms\\/.*/``. In practice it occasionally writes the visually
    obvious shorthand ``/rooms/.*`` or even ``text=/rooms/.*``. The generic
    selector parser quite reasonably treats that as a literal string, which
    makes a URL such as ``/rooms/64abc`` fail even though the authored intent
    is unambiguous.
