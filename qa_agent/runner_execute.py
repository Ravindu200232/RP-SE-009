...
class RunnerExecutionMixin:
    ...
    def _path_batches(self, paths) -> list:
        ...
        The old runner concatenated every repaired test file into one command.
        CommandRunner deliberately rejected strings over a tiny safety limit,
        so a *larger successful repair* paradoxically made QA disappear.  Keep
        every requested test, but execute as several ordinary Vitest invocations
        when the command would become unwieldy.
        """
