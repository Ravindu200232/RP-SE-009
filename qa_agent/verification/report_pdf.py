...
A deliberate sibling of the SRS PDF: same palette, same table and badge
furniture, same page chrome. The two land in the same inbox, and looking like
one product matters more here than any individual choice in this file.

Everything the Testing tab can show goes in, because the reason to export a
test report is to hand it to somebody who cannot click the tabs — a client, an
auditor, the person deciding whether to ship. That includes the parts nobody
enjoys printing: the rounds where the suite was failing, the tests that had to
be repaired, the quarantined ones, and the performance score measured against
a dev server rather than a production build. A report that shows only the last
green run is an advert, not a test report.

The test sources go in an appendix. They are the evidence for every number in
front of them, and a reader who wants to know what "55 passed" actually
covered has nowhere else to look.

ReportLab is pure-Python, so this works on Windows with no native libraries.
"""
...
def _missing(story, st, what: str) -> None:
    ...
    `read_qa_results` is careful to report a stage that never ran differently
    from one that ran and found nothing, and throwing that away at the last
    step would make the document lie about the one thing it is for.
    """
...
def build_qa_pdf(qa: dict, out_path: str | Path, *, project: str = "") -> Path:
    """Render the whole testing tab into `out_path`. Returns the path."""
    qa = qa or {}
    ...
    e2e_stage_total = int(e2e.get("stage_total") or 0)
    e2e_stage_passed = int(e2e.get("stage_passed") or 0)
    e2e_stage_rate = round(e2e_stage_passed * 100 / e2e_stage_total) if e2e_stage_total else 0
    e2e_result = (f"{e2e_stage_passed}/{e2e_stage_total} stages ({e2e_stage_rate}%)"
                  if e2e_stage_total else
                  f"{e2e.get('passed', 0)} journey(s) passed / {e2e.get('failed', 0)} failure(s)")
    ...
    if not e2e.get("ran"):
        ...
        stage_total = int(e2e.get("stage_total") or 0)
        stage_passed = int(e2e.get("stage_passed") or 0)
        stage_rate = round(stage_passed * 100 / stage_total) if stage_total else 0
        story.append(_table(
            [[_P("Overall E2E stages", st, "cellb"),
              _P(f"{stage_passed}/{stage_total} passed ({stage_rate}%)" if stage_total else "—", st)],
             [_P("Journeys", st, "cellb"), _P(e2e.get("flow", "—"), st)],
             [_P("Journey failures", st, "cellb"), _P(e2e.get("failed", 0), st)],
             [_P("Repaired", st, "cellb"), _P(e2e.get("fixed", 0), st)]],
            [W * 0.28, W * 0.72], header=False))
        flows = e2e.get("flows") or []
        if flows:
            story.append(Spacer(1, 8))
            rows = [[_P("Journey", st, "cellb"), _P("Stages", st, "cellb"),
                     _P("Rate", st, "cellb"), _P("Status", st, "cellb")]]
            for flow in flows:
                total_n = int(flow.get("stage_total") or 0)
                pass_n = int(flow.get("stage_passed") or 0)
                rate_n = round(pass_n * 100 / total_n) if total_n else 0
                rows.append([_P(flow.get("title") or flow.get("flow") or "Journey", st),
                             _P(f"{pass_n}/{total_n}", st), _P(f"{rate_n}%", st),
                             _P("pass" if total_n and pass_n == total_n else "incomplete", st)])
            story.append(_table(rows, [W * 0.50, W * 0.16, W * 0.14, W * 0.20]))
