"""Code search, route mapping, decision parsing and deterministic fallback reasoning."""
...
class DebuggerReasoningMixin:
    ...
    def _semantic_words(text: str) -> set[str]:
        ...
        aliases = {"checkout":"checkout", "cart":"cart", "catalogue":"catalogue",
                   "catalog":"catalogue", "rate":"rating", "rated":"rating",
                   "review":"review", "reviews":"review", "stock":"stock",
                   "inventory":"inventory", "category":"category", "categories":"category"}
        return {aliases.get(x, x) for x in words}
    ...
    def _selector_intent(failure) -> tuple[str, set[str]]:
        ...
        Do not mix ``failure.name`` into this signal: scenario names often
        contain user data and unrelated business nouns, which previously let
        arbitrary page copy satisfy a failed selector.  Prefer the concrete
        locator reported by the browser (field/role/testid); fall back to the
        failure message only when its shape is unknown.
        """
    ...
    def _selector_dom_excerpt(packet: str, channel: str) -> str:
        ...
        A form-field miss must be proven by the structured ``fields:``
        inventory; a button/link role miss by ``controls:``.  Headings and
        visible prose cannot prove either one.  This removes a major source of
        false TEST_FIX verdicts on pages such as a checkout error screen.
        """
    ...
    def _field_required_by_plan(field_words: set[str], plan_text: str) -> bool:
        ...
        JSON plan objects contain keys such as ``name`` everywhere.  Treating
        those keys as product requirements made `field=name` look mandatory
        even when registration only asked for email/password.
        """
        ...
        # Strip JSON/property keys so only requirement values contribute.
