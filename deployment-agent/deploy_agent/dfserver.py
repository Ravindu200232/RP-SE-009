...
class APIHandler(SimpleHTTPRequestHandler):
    ...
    def _aws_action(self, action: str, body: dict[str, Any]) -> None:
        ...
        if action == "sso/select":
            ...
            profile_name = str(body.get("profile", "deployment-agent")).strip() or "deployment-agent"
            if body.get("persist_profile") is True:
                profile_name = persist_sso_profile(
                    profile_name,
            ...
            result["profile"] = remember_sso_selection(profile_name, result)
            return self._json(result)
        if action == "profile/status":
            return self._json(
                profile_status(
                    str(body.get("profile", body.get("aws_profile", ""))),
                    str(body.get("region", "")),
                )
            )
        if action == "preflight":
    ...
    def _aws_session_from(body: dict[str, Any]):
        ...
        return session_for_profile(profile, region)
    ...
    def _onboarding() -> dict[str, Any]:
        status = tool_status()
        profiles: list[str] = []
        aws_identities: dict[str, dict[str, Any]] = active_sso_identities()
        if status.get("aws", {}).get("installed"):
            ...
            profiles = list(dict.fromkeys(profiles + list(aws_identities)))
            for profile in profiles[:20]:

                if profile in aws_identities:
                    continue
