...
A copy of the standalone `server.py` with four things taken out and nothing
added — see `deploy_agent/bridge.py` for why each one goes:

  * the websocket server on :7835   events are read back over
                                    GET /api/runs/{id}/events?after=N instead
  * the static dashboard from web/   the UI is the studio's Deploy tab
  * the browser it opened at start   it is not the application any more
  * the folder picker                AgentForge already knows which project

The file is named `dfserver`, not `server`, for the same reason `dfagents` is
not `agents`: AgentForge has its own `server.py` at the root of sys.path and one of
the two would shadow the other.
"""
...
def _migrate_readiness(value: dict[str, Any]) -> None:
    ...
    The category is now provider-neutral. Rewrite it on read so historical runs
    keep rendering with a full score bar instead of an empty provider row.
    """
...
class APIHandler(SimpleHTTPRequestHandler):
    ...
    def _aws_action(self, action: str, body: dict[str, Any]) -> None:
        ...
        Nothing sensitive crosses this boundary: the browser sends a start URL
        and selections, and receives verification links, account/role names, and
...
def start_reconciliation() -> None:
    ...
    The startup pass runs before the UI is reachable so a stale active run can
    never block the operator's first deployment of that project.
    """
