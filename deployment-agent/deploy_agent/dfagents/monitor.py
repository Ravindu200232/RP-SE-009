...
def select_workflow_run(
    ...
    The reviewed commit SHA is authoritative: a repository can hold older
    successful deploys of the same workflow, and treating the newest run as
    the current one lets a stale success advance a fresh deployment. Fall back
    to "newest run that is not the one observed before pushing" only while the
    new run has not appeared in the API yet.
    """
    if head_sha:
        for item in runs:
...
class MonitorAgent:
    ...
    def _capture_evidence(self, run_id: str) -> None:
        """Capture the dashboard now that the run is live, and report what
        actually landed.

        This step used to announce "Masked deployment evidence is ready for
        export" on nothing more than a readiness score, without the exporter
        having run. The stage went green, the evidence tab stayed at 0
        captured, and the empty state blamed a run that had "stopped earlier"
        — a run that had in fact finished. Say what happened instead.
        """
        try:
    ...
    def _stacks(self, session, slug: str) -> list:
        ...
        Extracted from `_aws` so the ECS collector reads the same stack the
        same way rather than growing a second, drifting copy.
        """
    ...
    def _cloudwatch(self, session, run: dict, snapshot: dict[str, Any]) -> None:
        """The application's own log lines. Provider-neutral already.

        Whether the process runs on an instance or in a container, it writes to
        the same log group, so both collectors call this unchanged. An empty
        result is the normal AWS outcome and costs the 10 `monitoring` points.
        """
        repo = run["repo"]
    ...
    def _ecs(self, run: dict, snapshot: dict[str, Any]) -> None:
        ...
        Deliberately the same key: readiness scoring, the studio's provider
        sniff and the evidence exporter all decide what to render by asking
        which of `aws`/`vercel` is populated, and a third key would mean
        editing all three for no gain.

        `stacks` and the CloudWatch `logs` block are reused verbatim — both are
        already provider-neutral, the log group name coming from repo_state.
        What changes is `instances` → `services`/`tasks` and SSM `releases` →
