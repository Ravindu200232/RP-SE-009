...
The CLI is used for sign-in and for linking a project, because that is what it is
good at. Everything the dashboard needs to *read back* - deployments, build logs,
domains - and every state change that must be exact - promote, delete - goes
through REST instead: the CLI's subcommand names and JSON shapes have moved
between versions, while these endpoints have not.
"""
...
def upsert_env(
    ...
    The type is deliberately `encrypted`, not `sensitive`. Vercel's sensitive
    variables are write-only: they reach the running function but `vercel pull`
    returns the literal placeholder `[SENSITIVE]`, so a Next.js build that reads
    the value - collecting page data from a database, for instance - fails with
    whatever error that placeholder produces. `encrypted` is still encrypted at
    rest and hidden in the dashboard, but is decrypted for builds.
    """
    return _request(
        "POST",
...
def add_domain(token: str, project_id: str, domain: str, team_id: str = "") -> dict[str, Any]:
    """Adding a domain usually needs a DNS record the operator must create at
    their registrar, so the caller is expected to surface `verification`."""
    return _request(
