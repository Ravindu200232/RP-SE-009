...
The agent never asks the operator to paste a token for its own use: it reads the
one the Vercel CLI already stores, exactly as it relies on `gh auth` and the AWS
SSO cache. The CLI has moved that file between versions and platforms, so every
known location is probed rather than assuming one.
"""
...
def _candidate_auth_files() -> list[Path]:
    ...
    The CLI resolves this through `xdg-app-paths`, which on Windows emulates the
    XDG layout under %APPDATA%\\xdg.data - not %APPDATA% directly. Both are
    probed because the layout has changed between CLI versions.
    """
...
def vercel_identity(token: str) -> dict[str, Any]:
    """Who the token belongs to. Used to show the connection state, and to fail
    early with a clear message rather than deep inside a deployment."""
    import requests
