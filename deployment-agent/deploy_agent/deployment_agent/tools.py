...
def _windows_tool_directories() -> list[str]:
    ...
    GUI launchers on Windows do not always inherit the PATH written by an
    installer until the parent application is restarted.  Resolve the CLIs we
    support from their standard locations as well so a newly installed tool is
    immediately usable by the agent.
    """
    if os.name != "nt":
        return []
