...
Most of a deployment is identical for every target: create the GitHub repository,
apply the reviewed artifacts, commit, push, wait for the workflow. The only real
variance is "get the provider ready, and tell me what to put into GitHub" - which
is what `ProviderPrep` carries. Everything else that differs is a fact, not
behaviour, so it lives in the table below rather than in a subclass.
"""
...
def profile_for(value: Any) -> TargetProfile:
    ...
    Anything unrecognised - including a run persisted before targets existed -
    resolves to AWS EC2, which is what those runs actually used.
    """
