    ...
    def start(self, start_url: str, region: str) -> dict[str, Any]:
        ...
        try:
            registration = oidc.register_client(
                clientName=CLIENT_NAME, clientType=CLIENT_TYPE, scopes=SSO_SCOPES
            )
        ...
        except Exception as exc:  # noqa: BLE001 - translate AWS onboarding errors
            raw = redact_text(str(exc))
            if "invalidrequest" in type(exc).__name__.lower() or "invalid_request" in raw.lower():
                raise ValueError(
                    "AWS rejected the Identity Center start URL and sign-in region. "
                    "Use the exact AWS access portal/issuer URL and the Region where "
