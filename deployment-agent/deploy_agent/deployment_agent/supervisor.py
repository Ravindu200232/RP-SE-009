...
A deployment is driven by a daemon worker thread.  When the application exits
mid-deployment that thread disappears while the SQLite row stays in an active
state, so the project stays locked against every later deployment even though
GitHub Actions may already have finished successfully.

The reconciler closes that gap.  It never starts, retries or rolls back a
deployment; it only reads the authoritative remote state through the existing
monitor and moves the persisted row to match.

Analysis runs are swept too.  `Orchestrator._analyze` is also a daemon thread,
so a restart strands its run in ANALYZING.  That does not lock the project --
`_reserve_project` only consults `_PERSISTED_ACTIVE_STATES` -- but the run sits
in the UI list forever, presenting itself as still working.
"""
...
def reconcile_once(
    ...
    Returns one record per run whose state changed, so the caller can log or
    stream the outcome.
    """
...
def start_supervisor(
    ...
    Monitoring must not depend on the dashboard displaying one particular run,
    so this sweep is what keeps every active deployment advancing to LIVE.
    """
