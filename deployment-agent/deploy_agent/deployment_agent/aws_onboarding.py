...
def remember_sso_selection(profile_name: str, selection: dict[str, Any]) -> str:
    """Associate one in-app SSO selection with a local profile name."""
    profile = str(profile_name or "deployment-agent").strip() or "deployment-agent"
    reference = str(selection.get("credential_reference", "")).strip()
    if not reference:
        raise ValueError("AWS sign-in did not return deployment credentials")
    # Validate the opaque reference before advertising the profile as active.
    CREDENTIAL_VAULT.get(reference)
    with _ACTIVE_SSO_LOCK:
        _ACTIVE_SSO_PROFILES[profile] = {
            "credential_reference": reference,
            "account_id": str(selection.get("account_id", "")),
            "role_name": str(selection.get("role_name", "")),
            "region": str(selection.get("region", "")),
            "expires_at": int(selection.get("expires_at", 0) or 0),
        }
    return profile


def _active_sso_selection(profile_name: str) -> dict[str, Any] | None:
    profile = str(profile_name or "").strip()
    if not profile:
        return None
    with _ACTIVE_SSO_LOCK:
        value = dict(_ACTIVE_SSO_PROFILES.get(profile) or {})
    if not value:
        return None
    reference = str(value.get("credential_reference", ""))
    expired = int(value.get("expires_at", 0) or 0)
    if expired and expired <= int(time.time()) + 30:
        with _ACTIVE_SSO_LOCK:
            _ACTIVE_SSO_PROFILES.pop(profile, None)
...
def credential_reference_for_profile(profile_name: str) -> str:
    value = _active_sso_selection(profile_name)
    return str((value or {}).get("credential_reference", ""))


def active_sso_identities() -> dict[str, dict[str, Any]]:
    """Return non-secret identity metadata for currently usable in-app sessions."""
    with _ACTIVE_SSO_LOCK:
        profiles = list(_ACTIVE_SSO_PROFILES)
    result: dict[str, dict[str, Any]] = {}
    for profile in profiles:
        value = _active_sso_selection(profile)
        if not value:
            continue
        result[profile] = {
            ...
            "arn": "",
            "user_id": "",
            "role_name": str(value.get("role_name", "")),
...
def session_for_profile(
    profile_name: str = "",
    region: str = "",
    credential_reference: str = "",
):
    """Prefer an in-app SSO session, falling back to a normal boto3 profile."""
    reference = str(credential_reference or "").strip()
    if not reference:
        reference = credential_reference_for_profile(profile_name)
    if reference:
    ...
    if str(profile_name or "").strip():
        kwargs["profile_name"] = str(profile_name).strip()
    return _boto3().Session(**kwargs)


def profile_status(profile_name: str, region: str = "") -> dict[str, Any]:
    """Report whether a profile can actually call AWS, not merely whether it exists."""
    profile = str(profile_name or "").strip()
    if not profile:
        return {"authenticated": False, "profile": "", "error": "Choose an AWS profile"}
    active = active_sso_identities().get(profile)
    if active:
        return {"authenticated": True, "profile": profile, "identity": active}
    try:
        identity = session_for_profile(profile, region).client("sts").get_caller_identity()
    except Exception as exc:  # noqa: BLE001 - translated into operator guidance
        raw = redact_text(str(exc))
        lowered = raw.lower()
        if "sso token" in lowered or "token for" in lowered or "expired" in lowered:
            raw = f"AWS SSO session for profile '{profile}' is missing or expired. Sign in again, then retry."
        return {"authenticated": False, "profile": profile, "error": raw}
    return {
        "authenticated": True,
        "profile": profile,
        "identity": {
