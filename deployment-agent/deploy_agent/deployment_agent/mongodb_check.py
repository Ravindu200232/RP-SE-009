...
The most expensive failure in this pipeline is a deployment that reaches AWS,
passes its health check, and only then turns out to have an unusable database
URI - a wrong password, or an Atlas IP allowlist that does not include the
instance. This check surfaces all of that in a couple of seconds, before any
...
def check_mongodb_uri(uri: str, timeout_ms: int = 6000) -> dict[str, Any]:
    ...
    Never returns the URI or any credential in its output.
    """
