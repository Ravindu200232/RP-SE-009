...
def _prod_dir(root):
    """The same rule server.py uses: AGENTFORGE_PROJECTS if set, else beside
    the repo. Both bridges MUST agree with the server on this or the SRS
    staging folder and the deploy records end up in a directory the server
    never reads."""
    import os
