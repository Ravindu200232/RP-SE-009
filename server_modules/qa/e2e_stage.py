...
def _analyze_before_journeys(analyzer) -> int:
    ...
    A browser journey reports what it saw, never what is wrong, and the gap
    between those two is where the repair loop gets lost. Measured: a room
    page linked to `/checkout` with no query string while the checkout page
    read `searchParams.get('roomId')`. roomId was null, so the price was never
    fetched, the total stayed null, and the journey failed on a discount line
    that could not render. Every request in that run returned 200 -- nothing
    in the network evidence could point at the link -- and four agentic rounds
    went into rewriting the page the failure appeared on while the defect sat
    one navigation upstream.

    The analyzer reads both files at once and calls that a broken contract.
    Running it here fixes the class before a journey is authored against it,
    instead of diagnosing it backwards from a symptom. One pass, not a loop:
    the cost has to stay bounded, and anything this misses still reaches the
    per-journey repair behind it.
    """
    try:
...
def _e2e_rounds(agent, arch, proj_dir, qa, analyzer, out):
    ...
    _analyze_before_journeys(analyzer)
...
def _repair_contract_backed_selector_failure(agent, arch, proj_dir, analyzer,
    ...
    `testid=book-now` is absent on a room detail page *and* the plan contract
    says that page must reach /bookings/new.  That is no longer a test guess;
