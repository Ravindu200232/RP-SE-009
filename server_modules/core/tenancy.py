...
def may(user, kind: str, key: str = "") -> bool:
    ...
    if kind == "srs-job":
        return (SRS_JOBS.get(key) or {}).get("user") == user["id"]
    if kind == "job":
...
def start_run(target, args, *, project=None, user=None) -> None:
    ...
    def turn():
        ...
        def told(ahead):
            emit({"type": "log", "level": "INFO", **({"project": project} if project else {}),
                  "text": (f"⏳ {ahead} build{'s' if ahead != 1 else ''} ahead of yours on "
                           "this server - yours starts as soon as "
                           f"{'they finish' if ahead != 1 else 'it finishes'}.")})

        RUN_QUEUE.run(lambda: target(*args), user=(user or {}).get("id", ""),
                      project=project, kind=getattr(target, "__name__", ""), on_wait=told)
...
def cancel_mine() -> tuple:
    """Stop this person's run, or take theirs out of the line. (answer, status)"""
    user = acting() or {}
    left = RUN_QUEUE.leave(user.get("id", ""))
    active = RUN_QUEUE.active()
    if active and active["user"] == user.get("id"):
        out = cancel.request()
