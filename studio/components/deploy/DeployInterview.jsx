...
const INPUT = 'mt-1 w-full rounded-lg border border-white/15 bg-white/5 px-3 py-2 text-[12px] text-white outline-none focus:border-blue-500'
...
export default function DeployInterview({ project, target, value, onChange, redeploy }) {
  ...
  return <section className="mt-5 rounded-xl border border-white/10 p-4">
    <p className="text-[13px] font-semibold text-white">Deployment choices</p>
    <p className="mt-1 text-[11px] text-white/55">{redeploy ? 'This update uses your recorded repository and cloud destination.' : 'Choose names and provider settings before the agent starts.'}</p>
    <div className="mt-3 grid gap-3 sm:grid-cols-2">
      {[...COMMON, ...(PROVIDER[target] || [])].map(([key, label]) => <label key={key} className="text-[11px] text-white/70">
      ...
      <label className="text-[11px] text-white/70">GitHub visibility
        <select className={INPUT} disabled={redeploy} value={value.repository_visibility || 'private'} onChange={e => patch('repository_visibility', e.target.value)}>
          <option className="bg-[#121622]" value="private">Private</option><option className="bg-[#121622]" value="public">Public</option>
        </select>
      </label>
      {target === 'aws_ec2' && <label className="text-[11px] text-white/70">EC2 instance size
        <select className={INPUT} value={value.aws_instance_type || 't3.micro'} onChange={e => patch('aws_instance_type', e.target.value)}>
          {['t3.micro', 't3.small', 't3.medium', 't4g.micro', 't4g.small'].map(size => <option className="bg-[#121622]" key={size}>{size}</option>)}
        </select>
      </label>}
      {target === 'azure' && <label className="text-[11px] text-white/70">App Service plan size
        <select className={INPUT} value={value.azure_sku || 'B1'} onChange={e => patch('azure_sku', e.target.value)}>
          {['F1', 'B1', 'B2', 'B3', 'S1', 'P1v3'].map(size => <option className="bg-[#121622]" key={size}>{size}</option>)}
        </select>
      </label>}
    </div>
    <label className="mt-3 block text-[11px] text-white/70">GitHub README content (leave empty to keep the current README)
