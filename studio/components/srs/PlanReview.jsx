...
export default function PlanReview({ projectId, onGenerated, onCancel }) {
  ...
  return (
    <div className="mx-auto w-full max-w-[1040px] pb-8 text-white">
      ...
      {open.length > 0 && (
        <section className="mt-5 rounded-2xl border border-[#1877F2]/30 bg-[#1C252E] p-5 shadow-xl backdrop-blur-xl">
          <div className="flex items-start gap-3"><span className="grid size-9 place-items-center rounded-xl bg-[#1877F2] text-white shadow-[0_4px_12px_0_rgba(24,119,242,0.24)]"><MessageCircleMore className="size-4" /></span><div><p className="text-[14px] font-bold text-white">{blocking.length ? `${blocking.length} details still matter` : 'A few optional details'}</p><p className="mt-0.5 text-[11.5px] text-white/60">Answer them like a conversation. The planner rewrites only the relevant part.</p></div></div>
          <div className="mx-auto mt-5 max-w-[760px] space-y-5">
            {open.map((q, i) => <div key={q.question || i}>
              <div className="flex items-end gap-3"><span className="grid size-7 place-items-center rounded-lg bg-accent/20 border border-accent/30 text-[9px] font-bold text-accent">AF</span><div className="max-w-[78%] rounded-2xl rounded-bl-sm border border-line bg-[#141A21] px-4 py-3 text-[12.5px] leading-relaxed text-white shadow-sm">{q.question}</div></div>
              {(q.options || []).length > 0 && <div className="ml-10 mt-2.5 flex flex-wrap gap-2">{q.options.map(opt => { const on = (replies[q.question] || '').trim() === opt; return <button key={opt} disabled={phase === 'revising'} onClick={() => setReplies(r => ({ ...r, [q.question]: on ? '' : opt }))} className={cn('rounded-full px-3.5 py-1.5 text-[11.5px] font-medium transition', on ? 'bg-[#1877F2] text-white border border-[#1877F2] shadow-[0_4px_12px_0_rgba(24,119,242,0.24)]' : 'border border-white/10 bg-white/[.05] text-white/80 hover:bg-white/[.1] hover:text-white')}>{opt}</button> })}</div>}
              <div className="ml-auto mt-2 max-w-[72%] rounded-2xl rounded-br-sm border border-line bg-[#141A21] px-3.5 py-2.5 shadow-sm"><TextArea rows={2} value={replies[q.question] || ''} placeholder="Your answer…" disabled={phase === 'revising'} onChange={e => setReplies(r => ({ ...r, [q.question]: e.target.value }))} className="w-full resize-none bg-transparent text-[12px] leading-relaxed text-white outline-none placeholder:text-white/40" /></div>
            </div>)}
          </div>
          <div className="mt-5 flex justify-end"><Button variant="solid" className="rounded-xl bg-[#1877F2] hover:bg-[#0C44AE] text-white shadow-[0_8px_16px_0_rgba(24,119,242,0.24)]" disabled={!answeredCount || phase === 'revising'} onClick={() => generate(composeAnswers(open, replies, revision))}>{phase === 'revising' ? <><Loader2 className="size-3 animate-spin" /> Updating…</> : <><Check className="size-3" /> Apply {answeredCount} answer{answeredCount === 1 ? '' : 's'}</>}</Button></div>
        </section>
      )}

      <section className="mt-5 rounded-2xl border border-line bg-[#1C252E] p-5 shadow-xl backdrop-blur-xl">
        <div className="flex items-center gap-2"><PencilLine className="size-4 text-[#1877F2]" /><p className="text-[12.5px] font-semibold text-white">Change anything in the plan</p></div>
        <TextArea value={revision} rows={3} placeholder="For example: add a wishlist, rename the admin area, or change the checkout flow…" onChange={e => setRevision(e.target.value)} className="mt-3 w-full resize-none rounded-xl border border-white/10 bg-white/[.03] px-4 py-3 text-[12.5px] leading-relaxed text-white outline-none focus:border-[#1877F2]/50 placeholder:text-white/40 caret-[#1877F2]" />
        <div className="mt-3 flex justify-end"><Button variant="outline" className="rounded-xl border-line bg-white/[.05] text-white/80 hover:bg-white/[.1] hover:text-white" disabled={!revision.trim() || phase === 'revising'} onClick={() => generate(revision.trim())}>{phase === 'revising' ? <><Loader2 className="size-3 animate-spin" /> Updating…</> : 'Update plan'}</Button></div>
      </section>

      {error && <p className="mt-4 rounded-xl border border-red-500/30 bg-red-500/10 px-4 py-3 text-[12px] text-red-300">{error}</p>}

      <div className="sticky bottom-4 mt-6 rounded-2xl border border-line bg-[#1C252E] p-4 shadow-2xl backdrop-blur-2xl">
        <div className="flex flex-wrap items-center gap-3">
          ...
          <button disabled={!settled || phase === 'revising'} onClick={accept} className="inline-flex h-11 items-center gap-2 rounded-xl bg-[#1877F2] px-6 text-[12.5px] font-semibold text-white shadow-[0_8px_16px_0_rgba(24,119,242,0.24)] transition hover:bg-[#0C44AE] disabled:opacity-40">
...
function Section({ icon: Icon, title, subtitle, children }) {
  return (
    <section className="rounded-2xl border border-line bg-[#1C252E] p-5 shadow-xl backdrop-blur-xl">
      <div className="mb-4 flex items-start gap-3">
        <span className="grid size-9 place-items-center rounded-xl bg-[#1877F2]/15 border border-[#1877F2]/20 text-[#1877F2]"><Icon className="size-4" /></span>
