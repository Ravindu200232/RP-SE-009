  ...
  return (
    <div className={cn('flex min-h-0 flex-1 flex-col', hidden && 'hidden')}>
      <div className="flex shrink-0 items-center gap-2 border-b border-line
        ...
        {badge ? (
          <span className="shrink-0 rounded-[3px] px-[3px] py-px font-mono
                ...
                style={{ color: badge[0], border: `1px solid ${badge[0]}55`,
                         background: `${badge[0]}18` }}>
            {badge[1]}
          </span>
        ) : (
        ...
        <span className="min-w-0 flex-1 truncate font-mono text-[11px] text-muted">
          ...
          {isDirty && <span className="ml-1.5 text-warn">●</span>}
        </span>

        {error && (
          <span className="max-w-[280px] truncate text-[10px] text-bad"
        ...
        {isDirty && !streaming && (
          <>
            <button onClick={revert} title="Throw the edit away"
                    className="flex shrink-0 items-center gap-1 rounded border
                               border-line px-2 py-0.5 font-mono text-[9px]
            ...
            <button onClick={save} disabled={!!saving}
                    ...
                    className="flex shrink-0 items-center gap-1 rounded border
                               border-accent/50 bg-accent/10 px-2 py-0.5
                               font-mono text-[9px] text-accent transition-colors
        ...
        {!!liveFile && !follow && (
          <button onClick={() => useStore.setState({ follow: true })}
                  ...
                  className="shrink-0 rounded border border-accent/40 px-2 py-0.5
                             font-mono text-[8.5px] text-accent
      ...
      <div className="flex min-h-0 flex-1">
        <div className="w-[210px] shrink-0 overflow-y-auto border-r border-line
                        bg-panel">
            ...
            <div className="flex h-full min-h-0">
              ...
              <div ref={gutterRef}
                   className="shrink-0 select-none overflow-hidden border-r
                              border-line/60 bg-panel/40 pt-4 text-right
                              font-mono text-[11.5px] leading-[1.75] text-muted2">
                {lines.map((_, i) => (
